...
both forced by the Nira environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Nira
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — Nira scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    Nira substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
