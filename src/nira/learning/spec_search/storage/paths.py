...
must NEVER be inside the Nira source tree. ``resolve_spec_search_root``
walks up from this module's ``__file__`` looking for a ``pyproject.toml`` that
identifies the Nira source root, then refuses to operate if the resolved
root is inside it. Defense in depth — if a user accidentally points
``NIRA_HOME`` at the repo, the system fails loudly instead of silently
...
from nira.core.paths import ConfigurationError, get_config_dir
from nira.security.file_utils import secure_mkdir

# ``ConfigurationError`` is re-exported from ``nira.core.paths`` (it used
...
def _find_source_root() -> Path | None:
    ...
    Returns the directory containing the Nira ``pyproject.toml``, or
...
def _resolve_nira_home() -> Path:
    """Resolve the Nira home directory via the unified core resolver.

    Delegates to ``get_config_dir`` so spec-search honors the same env-aware
    resolution (NIRA_HOME and XDG) as the rest of the framework.
    """
...
def resolve_spec_search_root() -> Path:
    ...
    The root is ``$NIRA_HOME/learning`` (or ``~/.nira/learning``
    by default). Raises ``ConfigurationError`` if the resolved path lies
    inside the Nira source tree, to prevent dev artifacts from leaking
    into the repo.
    """
    home = _resolve_nira_home()
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"NIRA_HOME ({home}) is inside the source tree "
                f"({source_root}). Spec search refuses to write runtime "
                "artifacts inside the Nira repo. Set NIRA_HOME "
