...
A thin wrapper over a local git repository at ``<nira_home>/.git``.
...
from nira.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.nira/`` (or ``$NIRA_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Nira source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Nira source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@nira.local")
            self._git("config", "user.name", "Nira Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new nira home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
