...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.nira.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "nira_meta" the value is
# stored under manifest.metadata["nira"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "prerequisites": ("nira_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.nira.original_frontmatter,
        and merges metadata.nira.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize nira namespace
        oj_meta = dict(raw_metadata.get("nira") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.nira.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.nira.* into canonical fields (these override
        # top-level mappings since they are explicit Nira-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.nira.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the nira metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["nira"] = oj_meta
