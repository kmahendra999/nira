...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]nira init[/bold] to detect hardware and generate "
        "[cyan]~/.nira/config.toml[/cyan].\n"
        "  Or run [bold]nira quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]nira doctor[/bold] to check all engines.\n"
        "  Run [bold]nira quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]nira config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]nira model list[/bold] to see available models."
        )
    return (
