"""``nira scheduler`` -- task scheduling commands."""
...
def scheduler_run_task(agent_name: str, dry_run: bool) -> None:
    ...
    when Nira is not running as a persistent daemon.

    Example (launchd plist ProgramArguments):
        nira scheduler run-task proactive
    """
    console = Console()
    store = _get_store()
    try:
        ...
        if match is None:
            console.print(
                ...
                "Register it first with 'nira scheduler create'.[/yellow]"
            )
            return
