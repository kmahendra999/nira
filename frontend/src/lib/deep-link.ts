/**
 * deep-link.ts — Handler for nira:// deep links in the Tauri desktop app.
 *
 ...
 *   nira://research/{session_id}   → open a research session
 *   nira://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `nira://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `nira://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `nira:` scheme.
 *
 * @example
 * parseDeepLink("nira://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("nira://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "nira:") return null;

    // The URL constructor treats "nira://research/abc123" such that
