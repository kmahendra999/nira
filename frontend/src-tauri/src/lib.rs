...
struct BootPlan {
    ...
    /// ~/.nira/config.toml so `nira serve` picks it up.
    engine_host: Option<(String, String)>,
    /// Args appended after `uv run nira serve --port <port>`.
...
fn boot_plan(cfg: &InferenceConfig, ram_gb: f64) -> BootPlan {
    match cfg.kind {
        ...
        SourceKind::Custom => {
            ...
            // `model` may be empty if the config is malformed; `nira serve`
...
/// Find the Nira project root (contains pyproject.toml).
/// Checks NIRA_ROOT env var, walks up from the executable, then
/// probes common clone locations.
fn find_project_root() -> Option<std::path::PathBuf> {
    // 1. Explicit env var override
    if let Ok(root) = std::env::var("NIRA_ROOT") {
    ...
    let direct = [
        format!("{home}/Nira"),
        format!("{home}/projects/hazy/Nira"),
        format!("{home}/projects/Nira"),
        format!("{home}/src/Nira"),
        format!("{home}/Documents/Nira"),
        format!("{home}/Desktop/Nira"),
        format!("{home}/Developer/Nira"),
        format!("{home}/dev/Nira"),
        format!("{home}/Code/Nira"),
        format!("{home}/code/Nira"),
        format!("{home}/repos/Nira"),
        format!("{home}/github/Nira"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for Nira one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/Nira without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("Nira");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is Nira (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("nira")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + Nira server child processes
// ---------------------------------------------------------------------------
...
/// Rolling buffer holding the most recent ~16 KB of nira stderr.
///
/// Populated by a background drainer task spawned at boot so the pipe
/// never fills and back-pressures `nira serve`; consumed by the boot
...
struct BackendManager {
    ...
    nira: Option<ChildHandle>,
    nira_stderr_tail: StderrTail,
...
impl Default for BackendManager {
    fn default() -> Self {
        Self {
            ollama: None,
            nira: None,
            nira_stderr_tail: Arc::new(Mutex::new(Vec::new())),
            boot_task: None,
        }
    }
}
...
impl BackendManager {
    async fn stop_all(&mut self) {
        ...
        if let Some(ref mut h) = self.nira {
            h.kill().await;
        }
        self.nira = None;
...
impl Default for SetupStatus {
    fn default() -> Self {
        Self {
            ...
            detail: "Choose where Nira should run models.".into(),
            ollama_ready: false,
            server_ready: false,
            model_ready: false,
            error: None,
...
/// Outcome of waiting for `nira serve` to become healthy.
///
...
fn spawn_nira_stderr_drainer(mut stderr: tokio::process::ChildStderr, tail: StderrTail) {
...
async fn read_nira_stderr_tail(backend: &SharedBackend) -> String {
    let tail = backend.lock().await.nira_stderr_tail.clone();
    let bytes = tail.lock().await.clone();
    String::from_utf8_lossy(&bytes).trim().to_string()
}

/// Poll `nira serve` health, watching the child process state so we
/// never wait 10 minutes for a process that crashed in the first second.
async fn wait_for_nira_health(
    url: &str,
    timeout: Duration,
    ...
    {
        Ok(c) => c,
        Err(_) => return NiraStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    loop {
        ...
        let exit_status = {
            ...
            match mgr.nira.as_mut() {
                Some(h) => h.child.try_wait().ok().flatten(),
                None => None,
            }
        };
        if let Some(status) = exit_status {
            let stderr = read_nira_stderr_tail(backend).await;
            return NiraStartResult::EarlyExit {
                code: status.code(),
                stderr,
            };
        }
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                    ...
                    return NiraStartResult::ServiceUnavailable(body);
                }
        ...
        if tokio::time::Instant::now() >= deadline {
            return NiraStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
///
/// When the Nira desktop binary is shipped as an AppImage, the AppImage
...
fn rust_toolchain_install_hint() -> &'static str {
    "The desktop app needs the Rust toolchain to build `nira_rust`. \
...
fn format_extension_import_failure(root: &std::path::Path, stderr: &str) -> String {
    ...
    format!(
        "`nira_rust` is still not importable after building. Last output:\n\n{}\n\n\
         Run these manually for the full build log:\n\n\
           cd {}\n\
           {}\n\
           uv run python -c \"import nira_rust\"",
        if tail.is_empty() {
            "(no stderr output)"
        } else {
            &tail
...
async fn verify_nira_rust_extension(
    root: &std::path::Path,
    uv_bin: &str,
) -> Result<(), String> {
    let mut cmd = tokio::process::Command::new(uv_bin);
    cmd.args(["run", "python", "-c", "import nira_rust"])
    ...
    match cmd.output().await {
        ...
        Err(e) => Err(format!(
            "Could not verify `nira_rust`: {}. Verify uv is installed at `{}`.",
            e, uv_bin
        )),
    }
}
...
fn port_owner_hint() -> String {
    if cfg!(target_os = "windows") {
        format!("netstat -ano | findstr :{}", NIRA_PORT)
    } else {
        format!("lsof -i :{}", NIRA_PORT)
    }
}
...
fn format_port_unavailable(port: u16, reason: &str) -> String {
    format!(
        "Port {} is not available: {}. Stop the process using that port or \
         change the Nira port, then relaunch.\n\nTo identify it:\n  {}",
        port,
        reason,
        port_owner_hint(),
    )
...
fn check_nira_port_available() -> Result<(), String> {
    match std::net::TcpListener::bind(("127.0.0.1", NIRA_PORT)) {
        Ok(listener) => {
            drop(listener);
            Ok(())
        }
        Err(err) => Err(format_port_unavailable(NIRA_PORT, &err.to_string())),
    }
}
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if plan.launch_ollama {
        ...
        // Point `nira serve` at the user's endpoint by writing the engine
        // host into ~/.nira/config.toml (the env var alone is shadowed by
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("Nira");
        ...
        if target_path.exists() && !target_path.join("pyproject.toml").exists() {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "{} exists but is not a valid Nira project. \
                 Remove it and relaunch, or set NIRA_ROOT to the correct path.",
                clone_target,
            ));
            return;
        }
        ...
        {
            let mut s = status.lock().await;
            s.detail = "Downloading Nira (first launch)...".into();
        }
    ...
    // already started `nira serve` in a terminal and then launched the
    ...
    // override from config instead of hard-coding NIRA_PORT.
    {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        match client
            .get(format!("http://127.0.0.1:{}/health", NIRA_PORT))
            .send()
            .await
        {
                ...
                if !confirm {
                    ...
                    match pending_rollback.staged_credential_present() {
                        Ok(true) => {
                            let mut s = status.lock().await;
                            s.error = Some(format!(
                                "An API server is already running on port {}, but Nira cannot securely apply your new inference API key to a server it did not start. Stop that server, then try setup again.",
                                NIRA_PORT,
                            ));
                            return;
                        }
                    ...
                    s.phase = "ready".into();
                    s.detail =
                        format!("Connected to existing API server on port {}.", NIRA_PORT,);
                    s.server_ready = true;
                    s.model_ready = true;
                    s.ollama_ready = true;
                    return;
            ...
            Ok(resp) if resp.status() == reqwest::StatusCode::SERVICE_UNAVAILABLE => {
                ...
                s.error = Some(format!(
                    "An API server is already running on port {} but its \
                     ...
                     `nira serve`, wait for it to finish loading and relaunch. \
    ...
    if let Err(err) = check_nira_port_available() {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    // the user saw no error, the boot continued, `uv run nira serve`
    // then ran in an under-provisioned venv, and the user waited the full
    // 600s health-check window before getting "Nira server did not
    ...
    sync_cmd
        .args([
            ...
            "--extra",
            "inference-google",
            // nira_rust lives in a uv dependency group (not the published
    ...
    {
        let mut s = status.lock().await;
        s.detail = "Verifying Rust extension (nira_rust)...".into();
    }
    if let Err(err) = verify_nira_rust_extension(root, &uv_bin).await {
        let mut s = status.lock().await;
        s.error = Some(err);
        return;
    }
    ...
    let nira_child = spawn_owned_child(&mut cmd);
    ...
    match nira_child {
        Ok(mut child) => {
            ...
            let mut mgr = backend.lock().await;
            let tail = mgr.nira_stderr_tail.clone();
            mgr.nira = Some(ChildHandle { child });
            drop(mgr);
            if let Some(stderr) = stderr_handle {
                spawn_nira_stderr_drainer(stderr, tail);
            }
        }
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start nira server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the Nira repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    let server_url = format!("http://127.0.0.1:{}/health", NIRA_PORT);
    match wait_for_nira_health(&server_url, Duration::from_secs(600), &backend).await {
        NiraStartResult::Ready => {}
        NiraStartResult::ServiceUnavailable(body) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Nira server is running but the inference engine is not available \
                 (HTTP 503). This usually means the configured model couldn't be loaded.\n\n\
                 Check the server logs, or run 'uv run nira serve --port {}{}' \
        ...
        NiraStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Nira server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Nira repo is at {}\n\
        ...
        NiraStartResult::Timeout => {
            let stderr = read_nira_stderr_tail(&backend).await;
            let mut s = status.lock().await;
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Nira server did not become ready within 10 minutes. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Nira repo is at {}\n\
                ...
                format!(
                    "Nira server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
fn api_base() -> String {
    format!("http://127.0.0.1:{}", NIRA_PORT)
}
...
async fn start_backend(
    ...
    if read_configured_inference_config().is_none() {
        *status.lock().await = SetupStatus::default();
        return Err("Choose an inference source before starting Nira.".into());
    }
...
#[tauri::command]
async fn run_nira_command(args: Vec<String>) -> Result<String, String> {
    let uv_bin = resolve_bin("uv");

    let mut cmd_args = vec!["run".to_string(), "nira".to_string()];
    cmd_args.extend(args.iter().cloned());

    let mut cmd = tokio::process::Command::new(&uv_bin);
    cmd.args(&cmd_args);
    // Run from the project root so `uv run nira` resolves the Nira
    // project regardless of the app's launch cwd. In a packaged install the
    // cwd isn't the checkout, so without this `nira` isn't found and the
    ...
    if !is_serve {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("Failed to launch nira: {}", e))?;
    ...
    // `nira serve` is a long-running server that never exits. The old code
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("Failed to launch nira serve: {}", e))?;

    let tail: StderrTail = Arc::new(Mutex::new(Vec::new()));
    if let Some(stderr) = child.stderr.take() {
        spawn_nira_stderr_drainer(stderr, tail.clone());
    }
    ...
    let url = format!("http://127.0.0.1:{}/health", NIRA_PORT);
    ...
    loop {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(format!(
                "nira serve exited (code {:?}) before becoming healthy:\n{}",
                status.code(),
                stderr.trim()
            ));
        }
        if let Ok(resp) = client.get(&url).send().await {
            if resp.status().is_success() {
                ...
                return Ok(format!(
                    "nira serve is ready on http://127.0.0.1:{}",
                    NIRA_PORT
                ));
            }
        }
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "nira serve did not become healthy on port {} within 120s.",
                NIRA_PORT
            ));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
...
const SECURE_KEY_SERVICE: &str = "Nira Cloud Keys";
...
const PENDING_INFERENCE_API_KEY: &str = "NIRA_PENDING_INFERENCE_API_KEY";
...
async fn reload_cloud_keys_for_owned_backend(
    ...
    let reload_url = format!("http://127.0.0.1:{}/v1/cloud/reload", NIRA_PORT);
...
async fn reload_cloud_keys_for_owned_backend_at(
    ...
    let mut manager = backend.lock().await;
    let Some(handle) = manager.nira.as_mut() else {
        return;
    };
...
// ---------------------------------------------------------------------------
// Inference-source selection (~/.nira/inference.json)
// ---------------------------------------------------------------------------
...
/// Path to the inference-source config (~/.nira/inference.json).
...
/// Write the custom-endpoint host into ~/.nira/config.toml so
/// `nira serve` (which reads that file via load_config) points at it.
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("NiraOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("NiraOverlayPanel", sup).unwrap();
        ...
        if Class::get("NiraOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("NiraOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("NiraOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("NiraOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("NiraOverlayPanel").unwrap();
        ...
        let hcls = Class::get("NiraOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("NiraOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit Nira").build(app)?;
            ...
            #[cfg(target_os = "macos")]
            unsafe {
                native_overlay::create(include_str!("overlay.html"), NIRA_PORT);
            }
...
mod tests {
    ...
    fn uv_sync_rust_failure_mentions_toolchain() {
        let msg = format_uv_sync_failure(
            Path::new("C:\\Users\\me\\Nira"),
            Some(1),
            "maturin failed: linker `link.exe` not found while building nira-rust",
        );
        assert!(msg.contains("exit 1"));
    ...
    fn extension_import_failure_names_verification_command() {
        let msg = format_extension_import_failure(
            Path::new("C:\\Users\\me\\Nira"),
            "ModuleNotFoundError: No module named 'nira_rust'",
        );
        assert!(msg.contains("nira_rust"));
        assert!(msg.contains(DESKTOP_UV_SYNC_COMMAND));
        assert!(msg.contains("uv run python -c \"import nira_rust\""));
    ...
    fn owned_child_process_helper() {
        let Some(started) = std::env::var_os("NIRA_OWNED_CHILD_STARTED") else {
            return;
        };
        let completed = std::env::var_os("NIRA_OWNED_CHILD_COMPLETED").unwrap();
    ...
    async fn aborting_boot_kills_child_before_manager_registration() {
        ...
        let task = tokio::spawn(async move {
            ...
            .env("NIRA_OWNED_CHILD_STARTED", started_for_child)
            .env("NIRA_OWNED_CHILD_COMPLETED", completed_for_child)
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null());
