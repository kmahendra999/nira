...
5. Clones the Nira repository to `%LOCALAPPDATA%\Nira`
...
`NIRA_FORCE`. If you need richer control, save the script first
...
remove the task with `nira-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\Nira\src\deploy\windows\nira-service.ps1"
...
`nira-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:NIRA_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/nira/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/nira.service` | `deploy/launchd/com.nira.plist` | `deploy/windows/nira-service.ps1` (cmdlet-driven) |
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Nira\src\deploy\windows\nira-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Nira"
```
