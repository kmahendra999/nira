...
pub struct PySchedulerStore {
    inner: nira_scheduler::SchedulerStore,
}
...
impl PySchedulerStore {
    ...
    fn new(db_path: &str) -> Self {
        Self {
            inner: nira_scheduler::SchedulerStore::new(db_path),
        }
    }

    fn create_task(&self, name: &str, schedule_type: &str, schedule_value: &str) -> PyResult<String> {
        let st = nira_scheduler::ScheduleType::parse(schedule_type).ok_or_else(|| {
    ...
    fn update_status(&self, id: &str, status: &str) -> PyResult<bool> {
        let s = nira_scheduler::TaskStatus::parse(status).ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
...
pub fn parse_cron_next(expr: &str, after: f64) -> Option<f64> {
    nira_scheduler::parse_cron_next(expr, after)
}
