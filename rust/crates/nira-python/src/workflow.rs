...
pub struct PyWorkflowGraph {
    inner: nira_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: nira_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => nira_workflow::NodeType::Tool,
            "condition" => nira_workflow::NodeType::Condition,
            "parallel" => nira_workflow::NodeType::Parallel,
            "loop" => nira_workflow::NodeType::Loop,
            "transform" => nira_workflow::NodeType::Transform,
            _ => nira_workflow::NodeType::Agent,
        };
        let node = nira_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = nira_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<nira_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(nira_workflow::WorkflowBuilder::new(name)),
        }
    }
