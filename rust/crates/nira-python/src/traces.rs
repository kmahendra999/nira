...
pub struct PyTraceStore {
    pub inner: Arc<nira_traces::TraceStore>,
}
...
impl PyTraceStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => nira_traces::TraceStore::new(std::path::Path::new(p)),
            None => nira_traces::TraceStore::in_memory(),
        }
...
pub struct PyTraceCollector {
    inner: nira_traces::TraceCollector,
}
...
impl PyTraceCollector {
    ...
    fn new(store: &PyTraceStore) -> Self {
        Self {
            inner: nira_traces::TraceCollector::new(Arc::clone(&store.inner)),
        }
    }
...
pub struct PyTraceAnalyzer {
    store: Arc<nira_traces::TraceStore>,
}
...
impl PyTraceAnalyzer {
    ...
    fn stats(&self) -> PyResult<String> {
        let analyzer = nira_traces::TraceAnalyzer::new(&self.store);
        let stats = analyzer
            .overall_stats()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
