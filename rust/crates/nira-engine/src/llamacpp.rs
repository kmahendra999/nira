...
use nira_core::error::{EngineError, NiraError};
use nira_core::{GenerateResult, Message, Usage};
...
impl InferenceEngine for LlamaCppEngine {
    ...
    fn generate(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<GenerateResult, NiraError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                NiraError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(NiraError::Engine(EngineError::Http(format!(
                "llama.cpp returned {status}: {body}"
            ))));
        }

        let data: Value = resp.json().map_err(|e| {
            NiraError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                NiraError::Engine(EngineError::Connection(e.to_string()))
            })?;
        ...
        let token_stream = byte_stream.filter_map(|chunk_result| async {
            match chunk_result {
                ...
                Err(e) => Some(Err(NiraError::Engine(EngineError::Streaming(
                    e.to_string(),
                )))),
            }
        });
    ...
    fn list_models(&self) -> Result<Vec<String>, NiraError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|_| {
                NiraError::Engine(EngineError::Connection(
