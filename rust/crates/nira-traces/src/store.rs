...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, NiraError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                NiraError::Io(std::io::Error::other(e))
            })?;
        }

        let conn = Connection::open(db_path).map_err(|e| {
            NiraError::Io(std::io::Error::other(e.to_string()))
        })?;
    ...
    pub fn save(&self, trace: &Trace) -> Result<(), NiraError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, NiraError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| {
                NiraError::Io(std::io::Error::other(
                    e.to_string(),
                ))
            })?;
    ...
    pub fn count(&self) -> Result<usize, NiraError> {
        let conn = self.conn.lock();
        let count: i64 = conn
