...
//!
//! Rust translation of `src/nira/learning/optimize/llm_optimizer.py`.
//!
...
/// Uses an LLM to propose optimal Nira configurations.
///
...
impl LLMOptimizer {
    ...
    pub fn analyze_trial(
        ...
        if let Some(ref backend) = self.backend {
            let prompt = format!(
                "Analyze this Nira evaluation result.\n\n\
                 ## Configuration\n{}\n\n\
                 ## Results\n- accuracy: {accuracy:.4}\n\
    ...
    fn build_initial_prompt(&self) -> String {
        ...
        lines.push("You are optimizing a Nira AI system configuration.".into());
