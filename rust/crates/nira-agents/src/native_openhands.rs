...
use nira_core::{AgentContext, AgentResult, NiraError, ToolResult};
use nira_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    NiraError::Agent(nira_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<nira_engine::Engine>>;
