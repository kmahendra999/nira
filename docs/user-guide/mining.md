...
Nira can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
Nira falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run nira mine init --cuda-visible-devices 0
```
...
- `nira mine models` lists Pearl model support status.
- `nira mine inspect-model` checks a Pearl model artifact before GPU launch.
- `nira mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `nira mine init` writes the local mining config and resolves the image.
- `nira mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `nira mine stop` stops the provider and removes the sidecar.
- `nira mine status` reads live gateway metrics.
- `nira mine attach` writes a sidecar for a miner you launched manually.
- `nira mine logs` prints the Docker container log tail.
- `nira mine validate-model` probes the active vLLM miner and gateway before
...
Nira only lists Pearl-compatible models published by the Pearl Research
