...
The channels module lets Nira send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from nira.channels.telegram import TelegramChannel
...
from nira.channels.slack import SlackChannel
...
The `nira channel` subcommand group provides quick access to channel operations.
...
nira channel send telegram "Build completed successfully"
```
...
When `nira serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.nira/config.toml`. Each platform has its own nested sub-section.
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from Nira!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  Nira
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. Nira sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since Nira runs locally, you need a tunnel so SendBlue can reach your webhook:
...
# Send a message
ok = channel.send("+15551234567", "Hello from Nira!")
```
...
nira channel status --channel-type sendblue

# Send a message
nira channel send sendblue "+15551234567" "Hello from Nira!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `nira serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.nira/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from nira.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Nira",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.nira/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.nira/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Nira"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.nira/whatsapp_baileys_bridge/auth"
assistant_name = "Nira"
...
- [API Reference: Channels](../api-reference/nira/channels/index.md) -- full class and type signatures
