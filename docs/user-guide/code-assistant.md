...
This writes a pre-configured `~/.nira/config.toml` for the code assistant.
...
```bash
nira ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
nira ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
nira ask "Read main.py and explain the architecture"

# Fix a bug
nira ask "Find and fix the bug in test_utils.py"

# Run tests
nira ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
nira ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
```bash
# Write a new script
nira ask "Write a Python script that converts YAML to JSON"

# Explain existing code
nira ask "Read src/nira/core/events.py and explain the EventBus pattern"

# Debug a failing test
nira ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
nira ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
nira ask "Read src/nira/tools/calculator.py and write unit tests for it"

# Shell tasks
nira ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`nira chat`).
- For stronger isolation, use the sandboxed agent: `nira ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `nira`. Use `cd /path && command` in your prompt if needed, or run `nira` from the project directory.
