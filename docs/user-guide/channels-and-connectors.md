...
Nira has two types of integrations:
...
```json
{
    ...
    "features": {
        ...
        "bot_user": { "display_name": "Nira", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @Nira in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Nira" |
...
3. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect gmail_imap`
...
Slack serves two purposes in Nira:
...
**DM your agent:**
- In Slack, find **Nira** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Nira"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Nira` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Nira` in the channel |
...
3. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect notion`
...
3. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Nira desktop app

2. Apple Notes is detected automatically when Full Disk Access is granted

Nira searches an indexed snapshot rather than querying Notes.app live.
...
3. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect outlook`
...
2. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect obsidian --path /path/to/vault`
...
4. **Connect in Nira:**
   ...
   - CLI: `uv run nira connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run nira deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.nira/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.nira/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
