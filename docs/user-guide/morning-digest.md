...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.nira/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.nira/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.nira/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." nira digest --fresh
```
...
```bash
nira digest --fresh          # Generate a new digest now
nira digest                  # Show today's cached digest
nira digest --text-only      # Print text without audio
nira digest --history        # Show past digests
nira digest --schedule "0 6 * * *"   # Set daily schedule
nira digest --schedule off   # Disable schedule
nira digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Nira (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/nira/prompts/personas/{name}.md`. The default `nira` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.nira/connectors/news_rss.json`:
...
**"No digest for today"** — Run `nira digest --fresh` to generate one.

**Empty sections** — Check connector status with `nira connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
