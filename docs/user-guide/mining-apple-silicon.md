...
README. Then write Nira's mining config:
...
```bash
nira mine doctor        # capability matrix
nira mine start         # launch gateway + miner-loop subprocesses
nira mine status        # check sidecar + gateway metrics
nira mine logs -n 120   # print recent logs
nira mine stop          # stop mining subprocesses
```
