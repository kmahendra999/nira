...
```bash
python examples/nira/spec_search_quickstart.py
```
...
`nira.learning.spec_search.composite_reward.score_batch`.
...
`configs/nira/examples/spec-search-quickstart.toml`. Copy it to
`~/.nira/config.toml` (or set `NIRA_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `nira.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `nira.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `nira.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `nira.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `nira.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
- `examples/nira/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/nira/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/nira/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/nira/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/nira/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/nira/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/nira/learning/spec_search/external_adapter.py` — corpus → trace adapter.
