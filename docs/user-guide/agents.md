...
Agents are the agentic logic layer of Nira. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
(The config directory honors `$NIRA_HOME` / `$XDG_DATA_HOME` when set.) The paths are configurable under `[memory_files]`:

```toml
[memory_files]
soul_path    = "~/.nira/SOUL.md"
memory_path  = "~/.nira/MEMORY.md"
user_path    = "~/.nira/USER.md"
...
```
~/.nira/personas/<name>/SOUL.md
~/.nira/personas/<name>/MEMORY.md
~/.nira/personas/<name>/USER.md
```

Select one per invocation, or opt out entirely:

```bash
nira ask --persona work  "summarize my open PRs"
nira ask --persona none  "what is 2 + 2?"     # inject no persona
```
...
`SOUL.md`, `MEMORY.md`, and `USER.md` are plain Markdown -- open them in any editor. `MEMORY.md` and `USER.md` can also be updated by the agent itself through the `memory_manage` and `user_profile_manage` tools when those are enabled, so the agent can record a new fact mid-conversation. These tools always target the default `MEMORY.md` and `USER.md` (under `~/.nira/`), never a named persona's copies -- edit those by hand.
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from nira.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
nira ask --agent simple "What is the capital of France?"

# Orchestrator with tools
nira ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
nira ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
nira ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
nira ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
nira ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
nira ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ with npm on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.nira/claude_code_runner/` on first use.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.nira/claude_code_runner/` and installs the pinned Agent SDK if it is missing or outdated.
...
4. The Python side parses the output between `---NIRA_OUTPUT_START---` and `---NIRA_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what Nira tool-calling agents support.
...
```bash
# Via CLI
nira ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept Nira tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; Nira supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your Nira engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`nira/<model>`).
...
| `provider_id`       | `str`             | `"nira"`   | opencode provider id to register/use                     |
...
```python
from nira.agents.opencode import OpenCodeAgent
...
nira ask --agent opencode "Refactor the parser to use a state machine"
```
...
```python
from nira.agents.operative import OperativeAgent
...
```bash
# Via CLI
nira ask --agent operative "Check system status"
```
...
```python
from nira.agents.monitor_operative import MonitorOperativeAgent
...
```bash
# Via CLI
nira ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from nira.sandbox import ContainerRunner, SandboxedAgent
from nira.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="nira-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/nira/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"nira-sandbox:latest"`| Docker image to run                            |
