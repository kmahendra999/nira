...
This writes a pre-configured `~/.nira/config.toml` for the operative agent with scheduling support.
...
nira scheduler status <task-id>

# Pause / resume / delete a task
nira scheduler pause <task-id>
nira scheduler resume <task-id>
nira scheduler delete <task-id>

# Run a task immediately (outside its schedule)
nira scheduler run <task-id>
...
- **All data stays local**: State is stored in `~/.nira/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `nira scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `nira scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `nira memory stats`. Clear old entries with `nira memory clear --before 2026-01-01` if needed.
