...
Nira has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.nira/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.nira/telemetry.db")
...
nira telemetry stats -n 5          # Top 5 models only

# Export records
nira telemetry export              # JSON to stdout
nira telemetry export -f csv       # CSV to stdout
nira telemetry export -o data.json # JSON to file
nira telemetry export -f csv -o metrics.csv

# Clear all records
nira telemetry clear               # With confirmation prompt
nira telemetry clear --yes         # Without confirmation
```
