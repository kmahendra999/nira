...
- **Persistence:** SQLite database file (default: `~/.nira/memory.db`)
...
```python
from nira.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from nira.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `nira.tools.storage.*` imports.
...
[Source: docs/intro.md] Nira is a modular AI framework...
...
nira memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
nira memory search "machine learning"
