...
!!! note "Optional component"
    The scheduler is a standalone module (`nira.scheduler`). It is not wired into the default `Nira` / `NiraSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `nira scheduler start`.
...
The `nira scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
nira scheduler list --status active

# Paused tasks
nira scheduler list --status paused
```
...
nira scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
nira scheduler resume a3f9b12c4d8e
```
...
nira scheduler cancel a3f9b12c4d8e
```
...
nira scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.nira/scheduler.db")  # (1)!

# Wire in a NiraSystem for task execution
...
4. Starts a daemon thread named `"nira-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.nira/config.toml`.

```toml title="~/.nira/config.toml"
[scheduler]
enabled = false
db_path = "~/.nira/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.nira/scheduler.db`     | SQLite database path                       |
