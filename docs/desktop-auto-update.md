...
The Nira desktop app ships with [Tauri's updater
...
on launch / every 30 min
        │
        ▼
GET https://github.com/kmahendra999/nira/releases/download/desktop-latest/latest.json
        │
        ▼
...
For frontend development, set `VITE_NIRA_NO_UPDATER=1` in your
...
```bash
export VITE_NIRA_NO_UPDATER=1
npm run tauri dev
```

This is purely a dev escape hatch — it has no effect on production
builds (where `import.meta.env.VITE_NIRA_NO_UPDATER` will be
...
curl -fsSL https://github.com/kmahendra999/nira/releases/download/desktop-latest/latest.json | jq .
