...
!!! tip "Prerequisites"
    ...
    - Nira installed: run `uv sync --extra dev` from the repository root
...
The script creates a `Nira` instance and delegates the research task to an `OrchestratorAgent` with five tools wired in. The orchestrator iterates through multiple tool-calling turns, deciding at each step whether to search, store, think, or synthesize.

```mermaid
sequenceDiagram
    participant U as User
    participant J as Nira SDK
...
j = Nira(model="qwen3:8b", engine_key="ollama")  # (1)!
try:
    response = j.ask(
...
1. Creates a `Nira` instance targeting the local Ollama engine with `qwen3:8b`. Both parameters are optional — omitting them uses auto-detected defaults from `~/.nira/config.toml`.
...
This is useful when you want to version-control the research configuration, share it with collaborators, or feed it to the `nira eval` runner for benchmarking.
...
Run `nira agent info orchestrator` to see the full tool catalog.
...
- [Getting Started: Configuration](../getting-started/configuration.md) — how to configure engines and models in `~/.nira/config.toml`
