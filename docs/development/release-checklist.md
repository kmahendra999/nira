...
2. **`nira` (no args)** drops into a chat session within 2 s.
...
6. **`nira doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`nira-uninstall`** removes `~/.nira/` and `~/.local/bin/nira*`. Verify with `ls`.
...
- Delete `~/.nira/config.toml` — verify bare `nira` re-runs init.
- Delete `~/.nira/.venv` — verify re-running curl heals it.
