...
```bash
uv run nira --version   # Should print 0.1.0
uv run nira --help      # Show all subcommands
```
...
Nira uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=nira --cov-report=html
```
...
Nira uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/nira/`:
...
src/nira/
    ...
    core/                       # Shared infrastructure
        config.py               # NiraConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # nira ask
        init_cmd.py             # nira init
        model.py                # nira model list/info
        memory_cmd.py           # nira memory index/search/stats
        telemetry_cmd.py        # nira telemetry stats/export/clear
        bench_cmd.py            # nira bench run
        serve.py                # nira serve
```
...
try:
    import nira.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
