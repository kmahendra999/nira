...
prove Nira wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Nira.
...
- Docker container `nira-pearl-miner` starts.
- `~/.nira/runtime/mining.json` is written.
...
```bash
docker ps --filter name=nira-pearl-miner
cat ~/.nira/runtime/mining.json
uv run nira mine logs --tail 200
...
## Verify Nira Inference Uses Mining Endpoint

Run:

```bash
uv run nira mine doctor
uv run nira ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/nira/mining/_metrics.py`.
...
```bash
uv run nira mine stop
docker ps --filter name=nira-pearl-miner
test ! -e ~/.nira/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `nira ask` no longer routes through `vllm-pearl-mining` unless another
...
- Nira inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run nira mine doctor
uv run nira mine status || true
uv run nira mine logs --tail 300 || true
docker inspect nira-pearl-miner || true
