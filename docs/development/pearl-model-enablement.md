...
through Pearl's vLLM miner and Nira.

Nira can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Nira only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Nira-specific conversion
...
`nira mine start`, vLLM `/v1/models`, Nira inference routing, Pearl
gateway template refresh, and `nira mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Nira until we have clean
...
2. Convert the target model.
   ...
   Nira includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `nira mine inspect-model` and
   `nira mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `nira mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `nira mine status` parses those metrics.

5. Promote the model in Nira.
...
```text
src/nira/mining/_models.py
```

`nira mine models` renders that registry. Planned models are visible to users
...
- `nira mine inspect-model --model <pearl-model-id> --allow-planned`
- `nira mine init --model <pearl-model-id>`
- `nira mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `nira ask "Say hello in one sentence."`
- `nira mine status`
- `nira mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `nira mine validate-model --output` to the issue.
