...
Nira API server. It builds `deploy/docker/Dockerfile`, binds the server to
Render's default web-service port (`10000`), and uses OpenAI's `gpt-4o-mini`
through the cloud engine.

## Create the service

1. Open Render's **New Blueprint Instance** flow and select your Nira
   fork.
...
4. Retrieve the generated `NIRA_API_KEY` from the Render dashboard and
...
for all of them. To use another provider, install its corresponding Nira
...
!!! warning "Free instances do not preserve Nira data"
    Render's free web-service filesystem is ephemeral. Everything written to
    `/home/nira/.nira` — including configuration, credentials,
...
persistent disk at `/home/nira/.nira`, or configure the relevant
...
preserve the default Nira home.
