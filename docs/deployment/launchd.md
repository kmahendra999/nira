...
Nira includes a launchd property list (plist) for running the API server as a background service on macOS. This provides automatic startup at login, automatic restart if the process exits, and log capture.
...
```bash
git clone https://github.com/nira-ai/nira.git && cd Nira && uv sync --extra server
...
```bash
cp deploy/launchd/com.nira.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.nira.plist
```
...
!!! note "Binds loopback by default"
    ...
    `EnvironmentVariables` block to set `NIRA_API_KEY`
    (`nira auth generate-key`); an unauthenticated `0.0.0.0` server refuses
    to start.

Verify it is running:

```bash
launchctl list | grep nira
```

You should see a line with the PID and the label `com.nira`. A `0` in the status column indicates the service is running normally.
...
The provided plist file at `deploy/launchd/com.nira.plist`:
...
<dict>
    <key>Label</key>
    <string>com.nira</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/bin/nira</string>
        <string>serve</string>
        <string>--host</string>
    ...
    <string>/tmp/nira.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/nira.stderr.log</string>
</dict>
</plist>
```
...
|----------------------|--------------------------------|------------------------------------------------------------------------------------------------------|
| `Label`              | `com.nira`               | Unique identifier for the service. Used with `launchctl` commands to manage the service.             |
| `ProgramArguments`   | `["/usr/local/bin/nira", "serve", "--host", "127.0.0.1", "--port", "8000"]` | The command and arguments to execute. Binds loopback by default; see the note above to expose on the LAN with an API key. |
...
| `StandardOutPath`    | `/tmp/nira.stdout.log`   | File where standard output is written. Contains server startup messages and access logs.             |
| `StandardErrorPath`  | `/tmp/nira.stderr.log`   | File where standard error is written. Contains error messages and stack traces.                      |
...
cat /tmp/nira.stdout.log

# View standard error (errors, warnings)
cat /tmp/nira.stderr.log

# Follow logs in real time
tail -f /tmp/nira.stdout.log /tmp/nira.stderr.log
```
...
!!! tip "Persistent log location"
    ...
    ```xml
    <key>StandardOutPath</key>
    <string>/Users/yourname/.nira/nira.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/yourname/.nira/nira.stderr.log</string>
    ```
...
launchctl unload ~/Library/LaunchAgents/com.nira.plist
```
...
```bash
# Stop the service
launchctl stop com.nira

# Start the service
launchctl start com.nira
```
...
```bash
# List all loaded services matching "nira"
launchctl list | grep nira
```
...
| Label  | The service label (`com.nira`)                           |
...
<key>EnvironmentVariables</key>
<dict>
    <key>NIRA_ENGINE_DEFAULT</key>
    <string>ollama</string>
    <key>NIRA_OLLAMA_HOST</key>
    <string>http://localhost:11434</string>
</dict>
```

### Using a Different `nira` Binary Path

If `nira` is installed in a virtual environment or a non-standard location, update the first element of `ProgramArguments`:

```xml
<key>ProgramArguments</key>
<array>
    <string>/Users/yourname/.local/bin/nira</string>
    <string>serve</string>
    <string>--host</string>
    <string>0.0.0.0</string>
    <string>--port</string>
...
The instructions above install the service as a **user agent** (runs only when you are logged in). To run Nira as a system-wide daemon that starts at boot regardless of user login:
...
```bash
sudo cp deploy/launchd/com.nira.plist /Library/LaunchDaemons/
sudo chown root:wheel /Library/LaunchDaemons/com.nira.plist
sudo launchctl load /Library/LaunchDaemons/com.nira.plist
```
