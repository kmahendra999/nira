...
**By:** Nira maintainers; contact: [user fills in]. Nira is a fork of OpenJarvis and is not a Stanford project.
...
Hi Pearl team — we're [Nira](https://github.com/kmahendra999/nira), a local-first personal AI agent framework, forked from Stanford Hazy Research's OpenJarvis. We're working on a `mining` subsystem that lets Nira users mine Pearl through the agent framework. The first integration is the `vllm-miner`-on-H100/H200 path, which is straightforward. The second is Apple Silicon, where the situation is more interesting and we'd like to confirm a few things before we ship.
...
- License compatibility: Pearl is ISC; Nira is Apache-2.0. We don't see any conflict for either consumption (v1) or contribution (v3), but please flag if you do.
