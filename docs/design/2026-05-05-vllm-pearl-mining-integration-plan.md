...
**Goal:** Implement Spec A — the v1 vLLM-Pearl mining integration — as a new `nira.mining` subsystem that lets H100/H200 owners running vLLM solo-mine the Pearl PoUW chain with their normal LLM inference.

**Architecture:** New top-level subsystem `mining/` mirrors OJ's existing primitive pattern (peer to `engine/`, `agents/`). A `MiningProvider` ABC + `MinerRegistry` enables future hardware/engine paths (Apple Silicon, AMD, Ollama) without rewrite. The v1 `vllm-pearl` provider orchestrates Pearl's published Docker container; a runtime sidecar at `~/.nira/runtime/mining.json` decouples mining lifecycle from the existing vLLM engine class. Three deliberate seams (`submit_target` tagged-union, zero-valued `fee_bps`/`fees_owed`, reserved `mining/pools/`) leave room for v2 pool support without retrofit pain.
...
- Absolute imports only (`from nira.core.registry import ...`).
...
| Path | Responsibility |
|---|---|
| `src/nira/mining/__init__.py` | Package init; soft-imports providers via `try/except ImportError` |
| `src/nira/mining/_stubs.py` | `MiningProvider` ABC, `MiningCapabilities`, `MiningConfig`, `MiningStats`, `SoloTarget`/`PoolTarget` tagged union, `Sidecar` dataclass + read/write helpers |
| `src/nira/mining/_discovery.py` | `detect_providers()`, hardware/Docker/disk/pearld checks, wallet-format check |
| `src/nira/mining/_constants.py` | `PEARL_REPO`, `PEARL_PINNED_REF`, `PEARL_IMAGE_TAG`, default ports, default model, sidecar path constants |
| `src/nira/mining/_docker.py` | `PearlDockerLauncher` — `ensure_image()`, `start()`, `stop()`, `is_running()`, `get_logs()` |
| `src/nira/mining/_collector.py` | `MiningTelemetryCollector` — full impl, shipped unwired in v1; v1.x lights it up in the gateway daemon |
| `src/nira/mining/vllm_pearl.py` | `VllmPearlProvider` (`MiningProvider` impl); `_parse_gateway_metrics()`; `ensure_registered()` |
| `src/nira/mining/pools/__init__.py` | Reserved location for v2 `PoolClient` work — empty except for a docstring saying so |
| `src/nira/cli/mine_cmd.py` | `nira mine` Click group: `init`, `start`, `stop`, `status`, `doctor`, `attach`, `logs` |
...
| Path | Change |
|---|---|
| `src/nira/core/registry.py` | Add `MinerRegistry` class + entry in `__all__` |
| `src/nira/core/config.py` | Add `MiningConfig` field to `NiraConfig`; add TOML parser for `[mining]` with `submit_target` tagged-union resolution |
| `src/nira/engine/_discovery.py` | Add sidecar-aware engine resolution: when `~/.nira/runtime/mining.json` exists, register a derived `vllm` engine pointing at `vllm_endpoint` |
| `src/nira/telemetry/store.py` | Add nullable `mining_session_id` column to inference rows; bump `PRAGMA user_version`; helper to tag rows when sidecar is present |
| `src/nira/cli/__init__.py` | Register the `mine` Click group |
| `src/nira/cli/hints.py` | Add hint: "mining configured but not running — start it with `nira mine start`" |
...
**Files:**
- Modify: `src/nira/core/registry.py`
...
```bash
git add src/nira/core/registry.py tests/conftest.py tests/core/test_registry.py
...
**Files:**
- Create: `src/nira/mining/__init__.py`
- Create: `src/nira/mining/_constants.py`
- Create: `src/nira/mining/_stubs.py`
- Create: `src/nira/mining/pools/__init__.py`
...
from nira.core.config import GpuInfo, HardwareInfo
...
def test_mining_config_v1_defaults():
    from nira.mining._stubs import MiningConfig, SoloTarget
    cfg = MiningConfig(
        provider="vllm-pearl",
...
def test_mining_provider_is_abstract():
    from nira.mining._stubs import MiningProvider
...
```
Expected: ALL fail with `ImportError` on `nira.mining._stubs`.
...
RUNTIME_DIR = Path.home() / ".nira" / "runtime"
...
PEARL_CACHE_DIR = Path.home() / ".nira" / "cache" / "pearl"
```
...
from nira.mining._stubs import (
    MiningCapabilities,
    MiningConfig,
    MiningProvider,
    MiningStats,
...
try:
    from nira.mining import vllm_pearl  # noqa: F401
...
```python
# src/nira/mining/pools/__init__.py
...
```bash
git add src/nira/mining/ tests/mining/__init__.py tests/mining/conftest.py tests/mining/test_stubs.py
git commit -m "feat(mining): add ABC, dataclasses, constants, sidecar IO"
```

---

## Task 3 — `MiningConfig` integration in `NiraConfig` + TOML parsing

**Files:**
- Modify: `src/nira/core/config.py`
...
provider       = "vllm-pearl"
wallet_address = "prl1qexampleaddress"
submit_target  = "pool:https://pool.nira.ai/submit"
...
- [ ] **Step 4: Add `mining` field to `NiraConfig`**

In `src/nira/core/config.py`, add an import near the top:
...
Add the field to `NiraConfig` (insert in alphabetical order — after `memory_files`, before `operators`):
...
In `src/nira/core/config.py`, locate the existing `load_config()` function (around line 1541). Find the section that processes the loaded TOML data into the `NiraConfig` (look for where it iterates the `data` dict and assigns to dataclass fields). Add a dedicated handler for `mining` because of the tagged-union parsing — it can't go through the generic walker.
...
```bash
git add src/nira/core/config.py tests/core/test_config.py tests/mining/fixtures/
git commit -m "feat(mining): integrate MiningConfig into NiraConfig with TOML parsing"
```
...
**Files:**
- Create: `src/nira/mining/_discovery.py`
...
def test_detect_supported_on_h100(hopper_hw):
    from nira.mining._discovery import detect_for_engine_model
...
def test_check_docker_available_true():
    from nira.mining._discovery import check_docker_available
    with patch("nira.mining._discovery._docker_client") as fake:
...
def test_check_disk_free_passes(tmp_path):
    from nira.mining._discovery import check_disk_free
    with patch("nira.mining._discovery.shutil.disk_usage") as du:
        # 500 GB free
...
def test_check_pearld_reachable_true():
    from nira.mining._discovery import check_pearld_reachable
    with patch("nira.mining._discovery.httpx.post") as post:
...
def test_check_wallet_address_format_valid():
    from nira.mining._discovery import check_wallet_address_format
...
```bash
git add src/nira/mining/_discovery.py tests/mining/test_discovery.py
...
**Files:**
- Create: `src/nira/mining/_docker.py`
...
def test_ensure_image_already_local():
    ...
    fake = MagicMock()
    fake.images.get.return_value = MagicMock(id="sha256:abc", tags=["nira/pearl-miner:main"])
    launcher = PearlDockerLauncher(client=fake)
    out = launcher.ensure_image("nira/pearl-miner:main")
    assert out == "nira/pearl-miner:main"
    fake.images.get.assert_called_once_with("nira/pearl-miner:main")
...
def test_ensure_image_falls_back_to_build_for_default_tag():
    from nira.mining._docker import PearlDockerLauncher
    from nira.mining._constants import PEARL_IMAGE_TAG
    import docker.errors as derr
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
def test_ensure_image_errors_when_non_default_tag_missing():
    from nira.mining._docker import PearlDockerLauncher, ImageAcquisitionError
    import docker.errors as derr
    import pytest
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
```bash
git add src/nira/mining/_docker.py tests/mining/test_docker.py
...
**Files:**
- Modify: `src/nira/mining/_docker.py`
...
def test_launcher_start_calls_run_with_expected_kwargs(_env_password):
    ...
    cfg = MiningConfig(
        ...
        submit_target=SoloTarget(pearld_rpc_url="http://localhost:44107"),
        extra={
            "docker_image_tag": "nira/pearl-miner:main",
            "model": "pearl-ai/Llama-3.3-70B-Instruct-pearl",
            "vllm_port": 8000,
...
def test_launcher_start_errors_when_password_env_missing():
    from nira.mining._docker import PearlDockerLauncher, ConfigurationError
    from nira.mining._stubs import MiningConfig, SoloTarget
    fake = MagicMock()
    launcher = PearlDockerLauncher(client=fake)
    cfg = MiningConfig(
    ...
    with pytest.raises(ConfigurationError) as ei:
        launcher.start(cfg, image="nira/pearl-miner:main")
...
Append to `src/nira/mining/_docker.py`:
...
class ConfigurationError(RuntimeError):
    ...
    def start(self, config: "MiningConfig", image: str) -> Any:
        ...
        """
        from nira.mining._stubs import MiningConfig  # noqa: F401  (typing only)
...
**Files:**
- Modify: `src/nira/mining/vllm_pearl.py` (creating in next task — for now create the parser as a helper)
- Create: `src/nira/mining/_metrics.py`
...
```bash
git add src/nira/mining/_metrics.py tests/mining/test_metrics.py tests/mining/fixtures/
...
**Files:**
- Create: `src/nira/mining/vllm_pearl.py`
...
def test_vllm_pearl_detect_supported_on_h100(hopper_hw):
    from nira.mining.vllm_pearl import VllmPearlProvider
...
async def test_vllm_pearl_start_writes_sidecar(tmp_path, monkeypatch):
    from nira.mining.vllm_pearl import VllmPearlProvider
    from nira.mining._stubs import MiningConfig, SoloTarget, Sidecar

    sidecar_path = tmp_path / "mining.json"
    monkeypatch.setattr(
        "nira.mining.vllm_pearl.SIDECAR_PATH", sidecar_path
    )
...
async def test_vllm_pearl_start_pool_target_raises_not_implemented(monkeypatch, tmp_path):
    from nira.mining.vllm_pearl import VllmPearlProvider
    from nira.mining._stubs import MiningConfig, PoolTarget
    ...
    cfg = MiningConfig(
        provider="vllm-pearl",
        wallet_address="prl1qaaa",
        submit_target=PoolTarget(url="https://pool.nira.ai/submit"),
        extra={"docker_image_tag": "nira/pearl-miner:main"},
    )
...
async def test_vllm_pearl_stop_removes_sidecar(tmp_path, monkeypatch, written_sidecar):
    from nira.mining.vllm_pearl import VllmPearlProvider
    monkeypatch.setattr(
        "nira.mining.vllm_pearl.SIDECAR_PATH", written_sidecar
    )
    fake_client = MagicMock()
    provider = VllmPearlProvider(docker_client=fake_client)
...
def test_vllm_pearl_stats_reads_gateway(monkeypatch, written_sidecar):
    ...
    with patch("nira.mining.vllm_pearl.httpx.get") as get:
...
def test_ensure_registered_is_idempotent():
    from nira.core.registry import MinerRegistry
    from nira.mining.vllm_pearl import (
        VllmPearlProvider,
        ensure_registered,
    )
    ensure_registered()
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see nira#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
...
```bash
git add src/nira/mining/vllm_pearl.py tests/mining/test_vllm_pearl.py
...
**Files:**
- Modify: `src/nira/engine/_discovery.py`
...
Run:
```bash
sed -n '1,80p' src/nira/engine/_discovery.py
```
...
def test_engine_discovery_picks_up_mining_sidecar(tmp_path, monkeypatch, written_sidecar):
    ...
    """
    from nira.engine._discovery import discover_engines
    from nira.mining import _constants as mining_const
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    from nira.engine.openai_compat_engines import OpenAICompatEngine  # adjust to actual class name
...
```bash
git add src/nira/engine/_discovery.py tests/conftest.py tests/mining/conftest.py tests/engine/test_discovery.py
...
**Files:**
- Create: `src/nira/mining/_collector.py`
...
async def test_collector_collect_once_returns_stats(written_sidecar):
    from nira.mining._collector import MiningTelemetryCollector

    sample = (
        "pearl_gateway_shares_submitted_total 50\n"
        "pearl_gateway_shares_accepted_total 49\n"
    )
    with patch("nira.mining._collector.httpx.get") as get:
        get.return_value.status_code = 200
        get.return_value.text = sample
        store = MagicMock()
...
from nira.mining._metrics import parse_gateway_metrics
from nira.mining._stubs import MiningStats, Sidecar
...
```bash
git add src/nira/mining/_collector.py tests/mining/test_collector.py
...
**Files:**
- Modify: `src/nira/telemetry/store.py`
...
Run:
```bash
grep -n "PRAGMA user_version\|CREATE TABLE\|ALTER TABLE\|migrate" src/nira/telemetry/store.py | head -20
```
...
In `src/nira/telemetry/store.py`:
...
```bash
git add src/nira/telemetry/store.py tests/telemetry/test_store.py
git commit -m "feat(telemetry): add mining_session_id + mining_stats table"
```

---

## Task 12 — `nira mine doctor` (highest user-facing value, build first)

**Files:**
- Create: `src/nira/cli/mine_cmd.py`
...
def test_mine_doctor_prints_capability_matrix(monkeypatch):
    ...
    with patch("nira.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("nira.cli.mine_cmd.check_docker_available", return_value=(True, "running 24.0.7")), \
         patch("nira.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("nira.cli.mine_cmd.check_pearld_reachable", return_value=(True, "block height 442107 (synced)")):
...
def test_mine_doctor_flags_unsupported_hardware():
    ...
    with patch("nira.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("nira.cli.mine_cmd.check_docker_available", return_value=(True, "ok")), \
         patch("nira.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("nira.cli.mine_cmd.check_pearld_reachable", return_value=(False, "connection refused")):
        result = runner.invoke(mine, ["doctor"])
...
from nira.core.config import HardwareInfo, load_config
from nira.mining._constants import (
    DEFAULT_PEARL_MODEL,
...
def mine() -> None:
    ...
    See https://kmahendra999.github.io/nira/docs/user-guide/mining/ for the
    full guide.
    """
...
def doctor() -> None:
    ...
    if mining_cfg is not None:
        ...
        row("pearld", "RPC", False, "no [mining] config — run `nira mine init`")
...
```bash
git add src/nira/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): nira mine doctor"
```

---

## Task 13 — `nira mine init / start / stop`

**Files:**
- Modify: `src/nira/cli/mine_cmd.py`
...
def test_mine_start_runs_provider_start(monkeypatch):
    ...
    with patch("nira.cli.mine_cmd.MinerRegistry") as reg, \
         patch("nira.cli.mine_cmd.load_config") as load, \
         patch("nira.cli.mine_cmd.asyncio.run") as arun:
        from nira.mining._stubs import MiningConfig, SoloTarget
...
def test_mine_start_errors_when_no_mining_config():
    from nira.cli.mine_cmd import mine
    runner = CliRunner()
    with patch("nira.cli.mine_cmd.load_config") as load:
...
def init(
    ...
    if not cap.supported:
        raise click.ClickException(
            ...
            f"See `nira mine doctor` for details."
        )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            f"Set it before `nira mine start`.",
            err=True,
        )
    ...
    config_path = Path.home() / ".nira" / "config.toml"
...
"""
    ...
    click.echo(f"Done. Run `nira mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException("no [mining] section in config — run `nira mine init`")
    ...
    async def _run():
        await provider.start(cfg)
    asyncio.run(_run())
    click.echo(f"Mining started. Run `nira mine status` for live stats.")
...
```bash
git add src/nira/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): nira mine init/start/stop"
```

---

## Task 14 — `nira mine status / attach / logs`
...
def test_mine_status_renders_stats(written_sidecar, monkeypatch):
    from nira.cli.mine_cmd import mine
    runner = CliRunner()
    monkeypatch.setattr("nira.cli.mine_cmd.SIDECAR_PATH", written_sidecar)
    sample = (
        "pearl_gateway_shares_submitted_total 100\n"
        "pearl_gateway_shares_accepted_total 99\n"
    ...
    with patch("nira.mining.vllm_pearl.httpx.get") as get, \
         patch("nira.cli.mine_cmd.MinerRegistry") as reg:
        get.return_value.status_code = 200
        get.return_value.text = sample
        from nira.mining.vllm_pearl import VllmPearlProvider
...
def test_mine_attach_writes_sidecar(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nira.cli.mine_cmd.SIDECAR_PATH", sidecar)
...
def test_mine_logs_streams_container_output(monkeypatch):
    ...
    with patch("nira.cli.mine_cmd.PearlDockerLauncher",
...
def status() -> None:
    ...
    cfg = load_config().mining
    if cfg is None:
        raise click.ClickException("no [mining] section — run `nira mine init`")
    provider_cls = MinerRegistry.get(cfg.provider)
    provider = provider_cls()
    s = provider.stats()
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        container = docker.from_env().containers.get("nira-pearl-miner")
...
```bash
git add src/nira/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): nira mine status/attach/logs"
```
...
**Files:**
- Modify: `src/nira/cli/__init__.py`
- Modify: `src/nira/cli/hints.py`
...
```bash
sed -n '1,50p' src/nira/cli/__init__.py
grep -n "add_command\|@main.command\|main.add_command" src/nira/cli/__init__.py | head -20
```
...
def mining_not_running_hint(cfg, sidecar_present: bool) -> Optional[str]:
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running — start it with `nira mine start`"
```
...
```bash
git add src/nira/cli/__init__.py src/nira/cli/hints.py tests/cli/test_main.py tests/cli/test_hints.py
...
Nira can mine the [Pearl](https://github.com/pearl-research-labs/pearl)
...
uv run nira mine init    # writes [mining] config + builds Docker image (30-60 min first time)
uv run nira mine start
uv run nira mine status
```

## Diagnosing problems

`nira mine doctor` prints one row per check with a clear ✓ or ✗ and reason.
...
The `nira.mining` subsystem uses a registry pattern identical to
...
1. Create `src/nira/mining/<provider>.py`.
2. Subclass `nira.mining.MiningProvider`.
...
5. Add a soft-import in `mining/__init__.py`:
   ```python
   try:
       from nira.mining import <provider>  # noqa: F401
...
```markdown
- New mining providers must register via `MinerRegistry` in `src/nira/core/registry.py` and expose an idempotent `ensure_registered()` per the autouse-clear test convention.
```
...
```bash
uv run pytest tests/mining/ --cov=nira.mining --cov-report=term-missing
```
Expected: ≥ 80% coverage on the new `mining/` package.

- [ ] **Step 4: Smoke `nira mine doctor` on a non-mining dev box**

```bash
uv run nira mine doctor
```
...
- Runtime sidecar (`~/.nira/runtime/mining.json`) for engine ↔ mining handoff
- Telemetry: on-demand reads in v1; `MiningTelemetryCollector` shipped unwired for v1.x
- New CLI: `nira mine init|start|stop|status|doctor|attach|logs`
...
- [ ] Manual smoke: `uv run nira mine doctor` on a non-mining dev box prints honest output
