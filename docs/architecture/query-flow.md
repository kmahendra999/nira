...
This page traces the end-to-end journey of a user query through the Nira system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: nira ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: NiraConfig (hardware, engine defaults)
...
Nira supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
nira ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.nira/config.toml
```
...
- Returns a `NiraConfig` dataclass with all settings
...
```python
from nira.learning import ensure_registered
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.nira/telemetry.db`.
...
The `Nira` class in `sdk.py` provides the same query flow through a Python API:

```python
from nira import Nira

j = Nira(model="qwen3:8b", engine_key="ollama")
