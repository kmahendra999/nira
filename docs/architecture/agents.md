...
!!! note "Backward compatibility"
    The old `from nira.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.

```python
from nira.agents.native_react import NativeReActAgent
...
```python
from nira.agents.native_openhands import NativeOpenHandsAgent
...
```python
from nira.agents.openhands import OpenHandsAgent
...
Wraps the `@anthropic-ai/claude-agent-sdk` package via a bundled Node.js subprocess bridge. Unlike every other agent, inference is handled entirely by the Claude Agent SDK -- the Nira inference engine is not used. This makes `ClaudeCodeAgent` a true external agent, similar in spirit to `OpenHandsAgent` but implemented via subprocess rather than an importable Python SDK.
...
4. Reads stdout and extracts the JSON payload between `---NIRA_OUTPUT_START---` and `---NIRA_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `nira-sandbox=true`
