...
Channel implementations use Python's `threading` module rather than asyncio. This is a deliberate choice: Nira's core inference path is synchronous, and daemon threads are simpler to compose with synchronous code than coroutines.
...
1. Create a new file in `src/nira/channels/`.
...
```python
from nira.channels._stubs import BaseChannel, ChannelMessage, ChannelStatus
...
- [API Reference: Channels](../api-reference/nira/channels/index.md) -- complete class and type signatures
