...
description: The five-primitive architecture behind Nira — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Nira is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Nira Architecture](../assets/Nira_Architecture.png)
...
The sandbox module (`nira.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Nira use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/nira/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           NiraConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              nira ask -- query the assistant
        serve.py            nira serve -- start API server

    sdk.py              Nira class -- high-level Python SDK
