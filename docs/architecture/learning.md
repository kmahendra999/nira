...
!!! note "Backward compatibility"
    The canonical locations are now `nira.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `nira.core.types` (for `RoutingContext`). The old `nira.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `nira.learning._stubs`.
...
```bash
nira ask --router learned "What is the capital of France?"
```
...
```python
from nira.learning.router import HeuristicRouter, build_routing_context
...
```python
from nira.learning.router import build_routing_context
...
```python
from nira.learning.trace_policy import TraceDrivenPolicy
from nira.traces.analyzer import TraceAnalyzer
...
```python
from nira.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from nira.learning.sft_policy import SFTPolicy
```
...
```python
from nira.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from nira.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from nira.learning.heuristic_reward import HeuristicRewardFunction
...
over Nira's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `nira-learning`
