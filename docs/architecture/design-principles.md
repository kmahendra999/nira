...
Nira follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Nira is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Nira is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Nira(engine_key="ollama")  # Local Ollama server
...
Nira **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `nira init` and used as the default engine:

```bash
nira init --force
# Detects hardware, writes ~/.nira/config.toml with:
# [engine]
...
```bash
nira telemetry stats          # Aggregated statistics
nira telemetry export --json  # Export all records
```
...
Nira provides a **clean Python API** through the `Nira` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`nira[server]`, `nira[memory-colbert]`, etc.)
...
The API server (`nira serve`) implements the **OpenAI chat completions API format**, making Nira a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Nira:
...
Nira requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Nira.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `nira[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `nira[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `nira[memory-bm25]` | `rank-bm25` |
| API server | `nira[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `nira[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `nira[inference-vllm]` | `vllm` |
| PDF ingestion | `nira[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `nira[channel-whatsapp-baileys]` | Node.js 22+ |
