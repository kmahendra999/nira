...
2. **Tolerant pass** — maps non-spec vendor fields to canonical locations via `FIELD_MAPPING` table. Unmapped fields are logged and preserved in `metadata.nira.original_frontmatter`
...
Sidecar storage for optimization output at `~/.nira/learning/skills/<name>/optimized.toml`:
...
SkillManager.get_few_shot_examples()
  → system._skill_few_shot_examples (stashed on NiraSystem)
