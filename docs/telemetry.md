...
Nira ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the Nira team's PostHog instance —
...
[`src/nira/analytics/events.py`](../src/nira/analytics/events.py).
...
1. [`src/nira/analytics/redaction.py`](../src/nira/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/nira/analytics/events.py`](../src/nira/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- **Production target**: A self-hosted PostHog instance at
  `analytics.nira.ai`, Hetzner US-East. Single-tenant, operated
  by the Nira team.
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the Nira leaderboard. It is **not** the same as analytics and
