---
title: Code Snippets
description: Copy-paste patterns for common Nira tasks
---

# Code Snippets

Ready-to-use patterns for the most common Nira tasks. Each snippet is self-contained and copy-pasteable.
...
Run with: `nira compose run research_assistant "quantum computing advances"`

## API Server (1 command)

```bash
nira serve --port 8000 --engine ollama --model qwen3:8b
```
...
```bash
docker build -t nira .
docker run -p 8000:8000 nira serve --host 0.0.0.0
```
