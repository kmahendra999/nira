...
Nira on Windows installs two ways: **WSL2** (this page — the
...
- The first time you run `nira`, the WSL kernel may show a "process running in background" notification — that's the bg-orchestrator detaching. It's expected.
- Models are stored in WSL's filesystem (`~/.nira/`), not your Windows drive. To free up space later: `nira-uninstall` removes everything.
