---
title: Configuration
description: Complete reference for Nira configuration
---

# Configuration

Nira uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Nira creates the `~/.nira/` directory and populates it with a default config when you run `nira init`.

## Relocating the Nira directory

Nira keeps **all** of its state — config, databases, caches, logs,
...
`~/.nira`, but you can move it.

The root is resolved in priority order:

1. **`$NIRA_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/nira`** — used when `$XDG_DATA_HOME` is set (a single
   `nira` directory nested under it, per the XDG Base Directory spec).
3. **`~/.nira`** — the default. With no environment variables set, the
...
!!! note "Migration"
    ...
    existing installs. If you set `NIRA_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.nira`, Nira will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.nira "$NIRA_HOME"`.

`$NIRA_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.nira/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `nira init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Nira automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Nira uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
nira ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.nira/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `nira serve`.
...
```bash
nira serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.nira/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.nira/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.nira/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.nira/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `nira init`, Nira probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.nira/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Nira entirely from Python without a TOML file:
...
```python
j = Nira(config_path="/path/to/my-config.toml")
```
...
Nira respects the following environment variables:
...
| `NIRA_WEB_SEARCH_ENGINE` | Web search engine for the `web_search` tool: `auto` (default), `youcom`, `tavily`, or `duckduckgo`. |
