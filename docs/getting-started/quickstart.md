---
title: Quick Start
description: Get up and running with Nira in minutes
search:
  boost: 3
---
...
!!! tip "Running `nira` commands"
    Every `nira ...` example below assumes you have either activated the project venv
    (`source .venv/bin/activate`) or are prefixing each command with `uv run`. A bare
    `nira init --preset ...` from a fresh clone will fail with `command not found`.

## What You Can Build

Nira is a modular AI assistant framework. Here's what developers build with it:

=== "Chat with Any Model"

    ```bash
    nira ask "Explain quantum entanglement" -m qwen3.5:4b   # use qwen3.5:9b or larger on GPU
    ```

=== "Agent + Tools"

    ```bash
    nira ask --agent orchestrator --tools calculator,web_search "What is the GDP of France in USD?"
    ```
...
=== "Index Docs & Ask"
    ...
    nira ask "How do I configure the engine?"
    ```

    !!! warning "Requires the Rust extension"
        `nira memory index` and `nira memory search` import `nira_rust`. If you
        skipped the `uv run maturin develop -m rust/crates/nira-python/Cargo.toml`
        step in [Installation](installation.md), these commands fail with
        `ModuleNotFoundError: No module named 'nira_rust'`. Build the extension
...
=== "Morning Digest"
    ...
    ```bash
    cp configs/nira/examples/morning-digest-mac.toml ~/.nira/config.toml
    nira connect gdrive       # one OAuth flow for Gmail, Calendar, Tasks
    CARTESIA_API_KEY="..." nira digest --fresh
...
=== "Code Assistant"
    ...
    nira ask "Write a Python script that parses CSV files"
...
=== "Scheduled Monitor"
    ...
    nira scheduler start
    nira scheduler create \
...
Copy one of these to `~/.nira/config.toml` to get a pre-configured setup:

| Config | For | What it does |
|--------|-----|-------------|
| [`chat-simple.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/chat-simple.toml) | Any machine | Lightweight chat, no tools -- simplest setup |
| [`code-assistant.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/code-assistant.toml) | Any machine | Orchestrator agent with code execution, file I/O, shell |
| [`deep-research.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/deep-research.toml) | Any machine | Multi-hop research across indexed documents with citations |
| [`scheduled-monitor.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/scheduled-monitor.toml) | Any machine | Persistent operative agent on a cron schedule |
| [`morning-digest-mac.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/morning-digest-mac.toml) | Mac (Apple Silicon) | Daily spoken briefing from email, calendar, health, news |
| [`morning-digest-linux.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/morning-digest-linux.toml) | Linux / GPU server | Same, with vLLM support |
| [`morning-digest-minimal.toml`](https://github.com/kmahendra999/nira/blob/main/configs/nira/examples/morning-digest-minimal.toml) | Any machine | Just Gmail + Calendar |
...
This guide walks through the core workflows of Nira: the browser app, CLI, Python SDK, agents with tools, memory, benchmarks, and the API server.

!!! info "Prerequisites"
    Make sure you have [installed Nira](installation.md) and have at least one inference backend running (e.g., `ollama serve`).

## Browser App

The quickest way to experience Nira is the full chat UI running in your browser:
...
Set `NIRA_WEB_SEARCH_ENGINE` to pick an engine explicitly — `youcom`,
...
first if that is where you keep the key. Stop any existing Nira server
before restarting so it inherits the updated environment.

To stop all services, press ++ctrl+c++ in the terminal.

!!! tip "Environment variable"
    Set `NIRA_MODEL` to change the default model: `NIRA_MODEL=deepseek-r1:14b ./scripts/quickstart.sh`
...
This runs hardware auto-detection (GPU vendor, VRAM, CPU, RAM) and writes a config file to `~/.nira/config.toml` with sensible defaults for your system. It also selects the recommended inference engine.
...
The simplest way to interact with Nira is the `ask` command:

```bash
nira ask "What is the capital of France?"
```

Nira will auto-detect a running engine, select a model using the configured router policy, and return the response.

#### CLI Options

| Option | Description | Example |
|--------|-------------|---------|
| `-m`, `--model` | Override model selection | `nira ask -m qwen3:8b "Hello"` |
| `-e`, `--engine` | Force a specific engine | `nira ask -e ollama "Hello"` |
| `-t`, `--temperature` | Sampling temperature (default: 0.7) | `nira ask -t 0.2 "Hello"` |
| `--max-tokens` | Max tokens to generate (default: 1024) | `nira ask --max-tokens 2048 "Hello"` |
| `--json` | Output raw JSON result | `nira ask --json "Hello"` |
| `--no-stream` | Disable streaming | `nira ask --no-stream "Hello"` |
| `--no-context` | Disable memory context injection | `nira ask --no-context "Hello"` |
| `-a`, `--agent` | Use an agent | `nira ask -a orchestrator "Hello"` |
| `--tools` | Comma-separated tools | `nira ask --tools calculator,think "2+2"` |
| `--router` | Router policy for model selection | `nira ask --router heuristic "Hello"` |

### Via Python SDK

The `Nira` class provides a high-level Python interface:
...
```python
# Use default config (auto-detected hardware, ~/.nira/config.toml)
j = Nira()

# Override the model
j = Nira(model="qwen3:8b")

# Override the engine
j = Nira(engine_key="ollama")

# Use a custom config file
j = Nira(config_path="/path/to/config.toml")
```

!!! warning "Always call `close()`"
    The `Nira` instance holds references to telemetry stores and memory backends. Call `j.close()` when you are done to release resources.
...
```bash
nira ask --agent orchestrator --tools calculator,think "What is 137 * 42?"
```
...
Index a file or directory. Nira chunks the content and stores it in the configured memory backend (SQLite/FTS5 by default).
...
=== "CLI"
    ...
    nira memory index ./paper.txt --chunk-size 256 --chunk-overlap 32
    ```
...
=== "CLI"
    ...
    ```bash
    nira memory search "configuration options"
    nira memory search -k 10 "how to deploy"
    ```
...
When you have indexed documents, Nira automatically injects relevant context into your queries. The memory system searches for chunks matching your query and prepends them as system context before sending to the model.
...
=== "Specific benchmark"
    ...
    ```bash
    nira bench run -b latency
    nira bench run -b throughput
    ```
...
=== "Custom options"
    ...
    nira bench run -n 20 --json

    # Specific model and engine, write to file
    nira bench run -m qwen3:8b -e ollama -o results.jsonl
    ```
...
Nira provides an OpenAI-compatible API server for integration with existing tools and frontends.
...
Nira records telemetry for every inference call (timing, tokens, cost). View aggregated statistics:
...
```bash
nira telemetry export --format json
nira telemetry export --format csv -o telemetry.csv
```

Clear all telemetry records:

```bash
nira telemetry clear --yes
```
