"""Tests for ``nira doctor`` CLI command."""
...
from nira.cli.doctor_cmd import (
    CheckResult,
...
class TestDoctorRuns:
    def test_doctor_runs(self) -> None:
        ...
        with (
            patch("nira.cli.doctor_cmd.load_config", return_value=mock_config),
            patch(
                "nira.cli.doctor_cmd.DEFAULT_CONFIG_PATH",
                Path("/tmp/nonexistent/config.toml"),
            ),
            patch("nira.cli.doctor_cmd._check_engines", return_value=[]),
            patch("nira.cli.doctor_cmd._check_models", return_value=[]),
            patch(
                "nira.cli.doctor_cmd._check_speech_backend",
...
class TestCheckEngineProbing:
    def test_check_engine_probing(self) -> None:
        ...
        from nira.cli.doctor_cmd import CheckResult
    ...
    def test_full_doctor_health_checks_each_engine_once(self) -> None:
        ...
        with (
            patch("nira.cli.doctor_cmd._ensure_engines_imported"),
            patch("nira.cli.doctor_cmd._get_config", return_value=config),
            patch("nira.cli.doctor_cmd.load_config", return_value=config),
            patch(
                "nira.core.registry.EngineRegistry.keys",
                return_value=["beta", "alpha"],
            ),
            patch(
                "nira.engine._discovery._make_engine",
                side_effect=make_engine,
            ) as make,
            patch("nira.cli.doctor_cmd._check_python_version", return_value=ok),
            patch("nira.cli.doctor_cmd._check_config_exists", return_value=ok),
            patch("nira.cli.doctor_cmd._check_config_parses", return_value=ok),
            patch("nira.cli.doctor_cmd._check_optional_deps", return_value=[]),
            patch("nira.cli.doctor_cmd._check_speech_backend", return_value=ok),
            patch("nira.cli.doctor_cmd._check_nodejs", return_value=ok),
            patch("nira.cli.doctor_cmd._check_security_profile", return_value=ok),
        ):
...
class TestCheckDefaultModel:
    def test_empty_default_model_is_not_a_warning(self) -> None:
        ...
        mock_config = MagicMock()
        mock_config.intelligence.default_model = ""
        with patch("nira.cli.doctor_cmd.load_config", return_value=mock_config):
...
class TestCheckSpeechBackend:
    def test_check_speech_backend_ready(self) -> None:
        ...
        with patch(
            "nira.speech._discovery.get_speech_backend",
