...
from nira.cli import _version_check
from nira.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("NIRA_NO_UPDATE_CHECK", "CI", "NIRA_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.nira state.
    monkeypatch.setenv("NIRA_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_nira_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("NIRA_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NIRA_CONFIG", str(cfg))
    ...
    def test_nira_config_env_override(self, monkeypatch, tmp_path):
        """NIRA_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    ...
    def test_unknown_source_version_never_prompts_or_fetches(
        ...
        monkeypatch.setattr("nira.__version__", version)
        with patch("nira.cli._version_check._get_latest_version") as latest:
            _version_check._do_check()
        latest.assert_not_called()

    @patch("nira.cli._version_check._do_check")
    ...
    @patch(
        "nira.cli._version_check._do_check",
