...
from nira.cli._install_detect import InstallInfo, detect_install


def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``nira.__file__`` at ``tmp_path / nira / __init__.py``."""
    pkg_dir = tmp_path / "nira"
...
def test_editable_git_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "nira self-update"
    assert info.repo_root == repo


def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "nira" / "lib" / "python3.12"
    fake.mkdir(parents=True)
    _patch_pkg_file(fake, monkeypatch)

    info = detect_install()
    assert info.kind == "uv-tool"
    assert info.upgrade_command == "uv tool upgrade nira"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade nira"
...
def test_missing_nira_file_falls_back_to_pypi(monkeypatch):
    """nira unimportable / no __file__ — still get a sane default."""
    with patch("nira.cli._install_detect.Path") as mock_path:
