"""Tests for ``nira model convert`` command."""
...
model_module = importlib.import_module("nira.cli.model")
...
class TestModelConvert:
    ...
    def test_convert_mlx_quantize(
        ...
        assert "nira host" in result.output
    ...
    def test_convert_ollama_creates_model_from_gguf(
        ...
        assert "nira chat --engine ollama --model repo" in result.output
    ...
    def test_direct_hf_engines_are_noop(
        ...
        assert f"nira host some/repo --backend {engine}" in result.output
