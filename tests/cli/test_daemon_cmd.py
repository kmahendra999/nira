"""Tests for ``nira start|stop|restart|status`` daemon management commands."""
...
from nira.cli.daemon_cmd import _pid_alive, _read_pid, _write_pid
...
class TestDaemonCommands:
    ...
    def test_stop_no_server(self) -> None:
        """``nira stop`` when no PID file shows 'not running'."""
        with patch("nira.cli.daemon_cmd._read_pid", return_value=None):
            result = CliRunner().invoke(cli, ["stop"])
        assert result.exit_code != 0
        assert "No running server" in result.output

    def test_status_no_server(self) -> None:
        """``nira status`` when no PID file shows 'not running'."""
        with patch("nira.cli.daemon_cmd._read_pid", return_value=None):
    ...
    def test_write_and_read_pid(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nira.cli.daemon_cmd._PID_FILE", pid_file),
            patch("nira.cli.daemon_cmd.DEFAULT_CONFIG_DIR", tmp_path),
            patch("nira.cli.daemon_cmd._pid_alive", return_value=True),
        ):
            _write_pid(12345)
    ...
    def test_status_shows_running(self) -> None:
        """``nira status`` shows running info when PID exists."""
        mock_config = MagicMock()
        mock_config.server.host = "127.0.0.1"
        mock_config.server.port = 8000

        with (
            patch("nira.cli.daemon_cmd._read_pid", return_value=9999),
            patch(
                "nira.cli.daemon_cmd.load_config",
    ...
    def test_start_already_running(self) -> None:
        """``nira start`` exits with error when a server is already running."""
        with patch("nira.cli.daemon_cmd._read_pid", return_value=42):
...
class TestPidLiveness:
    ...
    def test_read_pid_stale_pid_returns_none(self, tmp_path: Path) -> None:
        ...
        with patch("nira.cli.daemon_cmd._PID_FILE", pid_file):
            assert _read_pid() is None
...
class TestDaemonDetachment:
    ...
    def _spawn_kwargs(platform: str) -> dict:
        ...
        """
        with (
            patch("nira.cli.daemon_cmd._read_pid", return_value=None),
            patch("nira.cli.daemon_cmd._write_pid"),
            patch("nira.cli.daemon_cmd.load_config"),
            patch("nira.cli.daemon_cmd.sys.platform", platform),
            patch("nira.cli.daemon_cmd.subprocess.Popen") as popen,
