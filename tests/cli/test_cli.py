...
import nira
from nira.cli import cli, main


class TestMainEntryPoint:
    """Tests for the ``nira`` console script entry point."""
    ...
    def test_windows_reconfigures_stdout_to_utf8(self) -> None:
        ...
        with (
            ...
            mock.patch("nira.cli.cli") as cli_mock,
        ):
            main()
...
class TestCLI:
    def test_help(self) -> None:
        ...
        assert "Nira" in result.output

    def test_version(self) -> None:
        result = CliRunner().invoke(cli, ["--version"])
        assert result.exit_code == 0
        assert nira.__version__ in result.output
    ...
    def test_preset_switch_requires_explicit_force(self, tmp_path: Path) -> None:
        ...
        assert "nira init --preset chat-simple --force" in " ".join(
...
class TestStartupResilience:
    ...
    down every `nira` command — including `nira serve` — because the CLI
    ...
    def test_importing_cli_does_not_import_numpy(self) -> None:
        ...
        assert result.returncode == 0, (
            "importing nira.cli pulled in numpy (a broken numpy would then "
