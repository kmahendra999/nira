"""Tests for the ``nira eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``nira eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``nira eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "nira-direct" in result.output
        assert "nira-agent" in result.output

    def test_eval_run_missing_args(self):
        """``nira eval run`` without required args fails gracefully."""
