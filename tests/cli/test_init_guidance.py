"""Tests for ``nira init`` next-steps guidance."""
...
from nira.cli.init_cmd import _next_steps_text
...
class TestInitShowsNextSteps:
    def test_init_shows_next_steps(self, tmp_path: Path) -> None:
        ...
        config_dir = tmp_path / ".nira"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nira.cli.init_cmd.PrivacyScanner"),
        ):
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "Getting Started" in result.output
        assert "nira ask" in result.output
        assert "nira doctor" in result.output
...
class TestNextStepsOllama:
    def test_next_steps_ollama(self) -> None:
        ...
        assert "nira ask" in text
        assert "nira doctor" in text
...
class TestMinimalConfig:
    def test_init_generates_minimal_by_default(self, tmp_path: Path) -> None:
        """Default nira init produces a short config."""
        config_dir = tmp_path / ".nira"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
        ...
        assert "nira init --full" in content

    def test_init_full_generates_verbose_config(self, tmp_path: Path) -> None:
        """nira init --full produces the full reference config."""
        config_dir = tmp_path / ".nira"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nira.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("nira.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nira.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nira.cli.init_cmd.PrivacyScanner") as MockScanner,
        ):
            from nira.cli.scan_cmd import ScanResult
            ...
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "nira scan" in result.output
