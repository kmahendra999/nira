...
def skill_cli(tmp_path):
    ...
    with (
        patch("nira.cli.skill_cmd._get_skill_paths", return_value=[tmp_path]),
        patch("nira.cli.skill_cmd.load_config", return_value=cfg),
        patch("nira.core.config.load_config", return_value=cfg),
        patch("nira.cli.skill_cmd.EventBus", return_value=bus),
    ):
...
def test_default_deny_blocks_real_shell_before_dispatch(skill_cli):
    ...
    with patch("nira.tools.shell_exec.ShellExecTool.execute") as execute:
...
def test_granted_shell_still_requires_user_confirmation(
    ...
    with patch(
        "nira.tools.shell_exec.ShellExecTool.execute",
