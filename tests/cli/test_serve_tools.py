"""Regression tests for tool selection during ``nira serve`` startup."""

from __future__ import annotations

import pytest

from nira.cli.serve import _resolve_allowed_tools, _resolve_server_cors_origins
...
def test_serve_defaults_tools_when_no_selection_is_configured():
    allowed, explicit = _resolve_allowed_tools(NiraConfig())
...
def test_cors_config_is_used_without_environment(monkeypatch):
    monkeypatch.delenv("NIRA_CORS_ORIGINS", raising=False)
