"""Tests for ``nira connect`` CLI command."""
...
def test_connect_list_no_connectors() -> None:
    ...
    runner = CliRunner()
    with mock.patch(
        "nira.cli.connect_cmd.connect.__wrapped__"
        if hasattr(cli, "__wrapped__")
        else "nira.core.registry.ConnectorRegistry.items",
        return_value=(),
    ):
        with mock.patch(
            "nira.core.registry.ConnectorRegistry.items",
            return_value=(),
        ):
...
def test_connect_specific_source(tmp_path: object) -> None:
    ...
    with (
        mock.patch(
            "nira.core.registry.ConnectorRegistry.contains",
            return_value=True,
        ),
        mock.patch(
            "nira.core.registry.ConnectorRegistry.get",
