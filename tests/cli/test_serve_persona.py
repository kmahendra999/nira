"""Regression: ``nira serve`` must wire the ``SystemPromptBuilder`` into the
...
# ``nira.cli.serve`` as a package attribute resolves to the click
...
def test_serve_wires_persona_builder_into_served_agent(
    ...
    with (
        patch("nira.server.app.create_app", side_effect=_capture_create_app),
