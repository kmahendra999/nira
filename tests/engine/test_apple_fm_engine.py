...
class TestSnapshotAccumulator:
    def test_cumulative_snapshots_become_deltas(self):
        from nira.engine._apple_fm_support import snapshot_deltas
...
class TestModelLabels:
    def test_known_labels_normalize(self):
        from nira.engine._apple_fm_support import validate_model_label
...
class TestOptionsAndSampling:
    def test_max_tokens_maps_to_sdk_name(self):
        from nira.engine._apple_fm_support import build_options_kwargs
    ...
    def test_sampling_defaults_to_greedy_for_reproducibility(self):
        from nira.engine._apple_fm_support import parse_sampling_spec
...
def _load_engine_module():
    ...
    """
    sys.modules.pop("nira.engine.apple_fm", None)
    return importlib.import_module("nira.engine.apple_fm")
...
class TestErrorMapping:
    ...
    def test_context_error_does_not_repeat_the_sdk_reason(self, engine_mod):
        from nira.engine._base import EngineContextLengthError
...
class TestOnRealHardware:
    def test_generates_with_real_token_counts(self):
        ...
        mod = importlib.import_module("nira.engine.apple_fm")
...
class TestAfmIsTreatedAsLocal:
    ...
    def test_image_privacy_guard_does_not_warn_for_afm(self):
        """`nira ask -i photo.png --engine afm` must not claim the image
        leaves the machine."""
        from nira.cli.ask import LOCAL_ENGINES
