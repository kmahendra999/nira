...
setup() {
    ...
    mkdir -p "$NIRA_HOME/.state"
    mkdir -p "$NIRA_HOME/src/rust/crates/nira-python"
    touch "$NIRA_HOME/src/rust/crates/nira-python/Cargo.toml"
...
@test "writes extension-failed marker on failure" {
    ...
    [ "$status" -ne 0 ]
    [ -f "$NIRA_HOME/.state/extension-failed" ]
    [ -s "$NIRA_HOME/.state/extension-failed" ]
}

@test "removes prior failed marker on success" {
    touch "$NIRA_HOME/.state/extension-failed"
    UV_STUB_EXIT=0 run bash "$SCRIPT"
    [ ! -f "$NIRA_HOME/.state/extension-failed" ]
    [ -f "$NIRA_HOME/.state/extension-built" ]
}
...
@test "calls uv run maturin develop with the right manifest" {
    ...
    grep -q "rust/crates/nira-python/Cargo.toml" "$UV_STUB_LOG"
}

@test "atomic rename — no .tmp file left behind" {
    UV_STUB_EXIT=0 run bash "$SCRIPT"
    leftover=$(find "$NIRA_HOME/.state" -name "*.tmp" 2>/dev/null | wc -l)
    [ "$leftover" -eq 0 ]
}
