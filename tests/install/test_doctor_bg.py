...
def test_doctor_shows_bg_section_when_state_present(tmp_nira_home: Path) -> None:
...
def test_doctor_exit_code_when_bg_failed(tmp_nira_home: Path) -> None:
    (tmp_nira_home / ".state" / "extension-failed").write_text("oom")
    runner = CliRunner()
    result = runner.invoke(doctor, [], catch_exceptions=False)
...
def test_doctor_no_bg_section_when_state_dir_empty(tmp_nira_home: Path) -> None:
