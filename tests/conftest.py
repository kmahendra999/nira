...
# Set BEFORE importing nira.core.config: DEFAULT_CONFIG_DIR /
# DEFAULT_CONFIG_PATH are resolved once at import time (the install-script
# model, where NIRA_HOME is set before the process starts). Without
# this, the very first import of nira.core.config in the session --
...
# reference them) to the developer's real ~/.nira for the rest of the
...
_MISSING = object()
_ORIGINAL_NIRA_HOME = os.environ.get("NIRA_HOME", _MISSING)
_ORIGINAL_NIRA_CONFIG = os.environ.get("NIRA_CONFIG", _MISSING)
_TEST_HOME = Path(tempfile.mkdtemp(prefix="nira-test-home-"))
_TEST_HOME_CLEANED = False
os.environ["NIRA_HOME"] = str(_TEST_HOME)
# An explicit config path takes precedence inside load_config(). It may point
# at a developer's real file even when NIRA_HOME is isolated, so remove
# it before config.py and its import-time constants are initialized.
os.environ.pop("NIRA_CONFIG", None)

from nira.core.config import GpuInfo, HardwareInfo, load_config  # noqa: E402
from nira.core.events import EventBus, reset_event_bus  # noqa: E402
from nira.core.registry import (  # noqa: E402
    AgentRegistry,
    BenchmarkRegistry,
    ChannelRegistry,
    CompressionRegistry,
...
def _cleanup_test_home() -> None:
    ...
    if _ORIGINAL_NIRA_HOME is _MISSING:
        os.environ.pop("NIRA_HOME", None)
    else:
        os.environ["NIRA_HOME"] = str(_ORIGINAL_NIRA_HOME)
    if _ORIGINAL_NIRA_CONFIG is _MISSING:
        os.environ.pop("NIRA_CONFIG", None)
    else:
        os.environ["NIRA_CONFIG"] = str(_ORIGINAL_NIRA_CONFIG)
...
def _no_update_check(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setenv("NIRA_NO_UPDATE_CHECK", "1")


@pytest.fixture(autouse=True)
def _isolated_nira_home(monkeypatch: pytest.MonkeyPatch) -> None:
    """Re-assert NIRA_HOME and clear load_config()'s cache per test.

    Two related leaks (#787): (1) something changing NIRA_HOME
    ...
    """
    monkeypatch.setenv("NIRA_HOME", str(_TEST_HOME))
    monkeypatch.delenv("NIRA_CONFIG", raising=False)
    load_config.cache_clear()
    yield
    load_config.cache_clear()
