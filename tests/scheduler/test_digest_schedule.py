...
class TestDigestScheduleCLI:
    """Tests for the ``nira digest --schedule`` flag."""
    ...
    def test_schedule_show_status(self):
        ...
        runner = CliRunner()
        with patch("nira.cli.digest_cmd.load_config", return_value=mock_cfg):
    ...
    def test_schedule_set_cron(self, tmp_path):
        ...
        with (
            patch(
                "nira.cli.digest_cmd.load_config",
                return_value=mock_cfg,
            ),
            patch(
                "nira.cli.digest_cmd.DEFAULT_CONFIG_PATH",
                config_path,
            ),
            patch(
                "nira.cli.digest_cmd._create_scheduler_task",
                return_value="abc123",
            ) as mock_create,
        ):
    ...
    def test_schedule_off(self):
        ...
        with (
            ...
            patch("nira.cli.digest_cmd._save_digest_schedule") as mock_save,
            patch(
                "nira.cli.digest_cmd._cancel_scheduler_tasks",
                return_value=1,
            ),
        ):
    ...
    def test_scheduler_task_persists_digest_timezone(self, tmp_path):
        from nira.cli import digest_cmd
        from nira.scheduler.store import SchedulerStore

        config_path = tmp_path / "config.toml"
        with patch("nira.cli.digest_cmd.DEFAULT_CONFIG_PATH", config_path):
...
class TestDigestScheduleEndpoints:
    ...
    def test_schedule_endpoint_returns_config(self, client):
        ...
        with patch(
            "nira.server.digest_routes.load_config",
            return_value=mock_cfg,
        ):
    ...
    def test_schedule_endpoint_update(self, client):
        ...
        with (
            ...
            patch("nira.server.digest_routes._save_digest_schedule") as mock_save,
            patch(
                "nira.server.digest_routes._create_scheduler_task",
    ...
    def test_schedule_endpoint_disable(self, client):
        ...
        with (
            ...
            patch("nira.server.digest_routes._save_digest_schedule") as mock_save,
            patch(
                "nira.server.digest_routes._cancel_scheduler_tasks",
                return_value=1,
            ) as mock_cancel,
        ):
            resp = client.post(
