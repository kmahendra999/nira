"""Tests for the Python SDK — Nira class and MemoryHandle."""
...
from nira.sdk import MemoryHandle, Nira
...
class TestNiraInit:
    ...
    def test_version_property(self):
        j = Nira(config=NiraConfig())
        assert j.version == nira.__version__
        j.close()

    def test_engine_key_override(self):
        j = Nira(config=NiraConfig(), engine_key="custom")
        assert j._engine_key == "custom"
        j.close()

    def test_model_override(self):
        j = Nira(config=NiraConfig(), model="my-model")
...
class TestNiraAsk:
    ...
    def test_simple_agent_ignores_tool_security_kwargs(self):
        ...
        with (
            patch("nira.sdk.get_engine", return_value=("mock", engine)),
            patch(
    ...
    def test_ask_no_engine_raises(self):
        with patch("nira.sdk.get_engine", return_value=None):
            j = Nira(config=NiraConfig())
...
class TestNiraLifecycle:
    ...
    def test_context_manager_closes_initialized_engine(self) -> None:
        engine = _make_engine()
        with patch("nira.sdk.get_engine", return_value=("mock", engine)):
            with Nira(config=NiraConfig()) as j:
                j.list_models()
            engine.close.assert_called_once()
