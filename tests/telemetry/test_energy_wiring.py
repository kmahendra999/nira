...
from nira.core.types import Message, Role, TelemetryRecord
from nira.telemetry.aggregator import AggregatedStats, TelemetryAggregator
from nira.telemetry.energy_monitor import BASIS_GPU, EnergySample
...
_bench_mod = importlib.import_module("nira.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a NiraConfig with energy monitoring enabled."""
    cfg = NiraConfig()
    ...
    # `nira ask` (no --agent) now falls back to ``agent.default_agent``
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "nira.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Nira instance."""
        from nira.sdk import Nira
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "nira.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "nira.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "nira.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "nira.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
