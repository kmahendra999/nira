...
def test_get_history(store, tmp_path):
    ...
    data = resp.json()
    assert len(data) == 1
    assert data[0]["voice_used"] == "nira"


def test_generate_runs_entire_nira_lifecycle_on_one_worker(tmp_path, monkeypatch):
    """Construction, ask, and cleanup all stay off the event-loop thread."""
    from nira.server import digest_routes
    ...
    monkeypatch.setattr("nira.sdk.Nira", FakeNira)
