...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("nira.mcp.transport.StreamableHTTPTransport"),
        patch("nira.mcp.client.MCPClient"),
...
def test_returns_tools_from_external_mcp_file(
    ...
    with (
        patch("nira.mcp.transport.StreamableHTTPTransport"),
        patch("nira.mcp.client.MCPClient"),
        patch("nira.tools.mcp_adapter.MCPToolProvider") as provider,
    ):
...
def test_app_shutdown_stops_scheduler_before_closing_shared_mcp_clients() -> None:
    ...
    from nira.server.agent_manager_routes import _start_managed_worker
...
def test_shutdown_interrupts_mcp_client_during_lazy_initialization() -> None:
    ...
    with (
        patch("nira.core.config.load_config", return_value=mcp_config),
        patch("nira.mcp.transport.StreamableHTTPTransport"),
        patch("nira.mcp.client.MCPClient", return_value=client),
    ):
        _start_managed_worker(
            app.state,
            _discover,
...
def test_app_shutdown_closes_lazily_created_memory_backend(monkeypatch) -> None:
    ...
    from nira.core.config import NiraConfig
    from nira.server import agent_manager_routes as routes
    from nira.server.app import create_app
...
def test_app_shutdown_keeps_owned_memory_open_for_live_worker(monkeypatch) -> None:
    ...
    from nira.core.config import NiraConfig
    from nira.server import app as app_module
    from nira.server.agent_manager_routes import _start_managed_worker
