...
from nira.core.types import Role  # noqa: E402
from nira.server.app import create_app  # noqa: E402
...
class TestChatCompletions:
    ...
    def test_multi_engine_local_fallback_reports_actual_ollama_route(self):
        ...
        with patch(
            "nira.server.cloud_router.stream_local",
...
def _identity_config():
    ...
    cfg = NiraConfig()
    cfg.agent.default_system_prompt = "You are Nira."
    cfg.analytics.enabled = False
    return cfg
...
class TestIdentityPromptInjection:
    ...
    saw Nira's identity system prompt and the model answered from its
    ...
    def test_stream_uses_grounded_agent_result_without_replay(self):
        ...
        captured: list = []
        engine = _make_capturing_engine(captured)
        agent = _make_agent(content="My name is Nira Prime.")
        agent._tools = [object()]
        ...
        assert streamed_content == "My name is Nira Prime."
        assert captured == []
    ...
    def test_agent_path_folds_history_into_one_identity_system_message(self):
        ...
        assert messages[0].content.count("You are Nira.") == 1
    ...
    def test_direct_merges_identity_and_auto_memory_into_one_system_message(self):
        ...
        assert "Nira" in system_messages[0].content
    ...
    def test_agent_does_not_rebuild_identity_already_merged_with_memory(self):
        ...
        system_messages = [m for m in messages if m.role == Role.SYSTEM]
        assert len(system_messages) == 1
        assert system_messages[0].content.count("You are Nira.") == 1
        assert "favorite color is blue" in system_messages[0].content
    ...
    def test_direct_injects_soul_persona_when_present(self, tmp_path):
        ...
        persona files never applied to this path, unlike ``nira ask`` and
        ...
        msgs = engine.generate.call_args.args[0]
        assert msgs[0].role.value == "system"
        assert "Nira" in msgs[0].content  # identity blurb still present
    ...
    def test_memory_context_does_not_suppress_identity_injection(self):
        ...
        from nira.server.routes import _ensure_identity_prompt
        from nira.tools.storage.context import build_context_message
        ...
        assert len(system_msgs) == 1
        assert "Nira" in system_msgs[0].content
        assert system_msgs[0].metadata["memory_context"] is True
        assert system_msgs[0].metadata["nira_identity_prompt"] is True
        ...
        assert normalized_again[0].content.count("You are Nira.") == 1
...
class TestModelsEndpoint:
    ...
    def test_configured_litellm_model_is_listed(self):
        ...
        with patch(
            "nira.server.cloud_router.list_local_models",
    ...
    def test_litellm_provider_model_streams_through_active_engine(self):
        ...
        with patch(
            "nira.server.cloud_router.stream_cloud",
...
def _traces_enabled_config(tmp_path):
    ...
    tests fail on any machine whose ``~/.nira/config.toml`` disables
