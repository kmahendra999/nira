...
from nira.security.ssrf import _check_ssrf_python, check_ssrf, is_private_ip
...
class TestCheckSsrf:
    ...
    def test_allows_normal_urls(self):
        ...
        with patch("nira.security.ssrf.socket.getaddrinfo") as mock_dns:
    ...
    def test_dns_failure_fails_closed(self):
        ...
        with patch(
            "nira.security.ssrf.socket.getaddrinfo",
...
class TestCheckSsrfPythonFallback:
    ...
    def test_falls_back_to_python_when_rust_unavailable(self):
        with patch("nira._rust_bridge.RUST_AVAILABLE", False):
            result = check_ssrf("http://169.254.169.254/latest/meta-data/")
        assert result is not None
        assert "cloud metadata" in result.lower() or "Blocked host" in result
