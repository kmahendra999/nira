...
from nira.core.config import NiraConfig, SkillSourceConfig
...
def test_install_uses_cached_skill_and_displays_warning(
    ...
    cached_skill(resolver)
    monkeypatch.setenv("NIRA_HOME", str(tmp_path / "installed"))
    with (
        patch("subprocess.run", side_effect=failed_pull),
        patch("nira.cli.skill_cmd._get_resolver", return_value=resolver),
    ):
