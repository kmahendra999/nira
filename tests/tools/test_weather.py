...
def test_registered_and_declared_external():
    import nira.tools.weather as weather_module
...
def test_tool_executor_applies_external_boundary_guard():
    ...
    with patch(
        "nira.tools.weather.weather_connector.fetch_weather",
...
def test_corrupt_connector_key_is_unconfigured_and_never_sent(
    ...
    monkeypatch.setattr("nira.tools.weather.get_tool_credential", lambda *_: None)
...
def test_secure_tool_credential_precedes_connector(monkeypatch):
    ...
    tool = WeatherTool(connector=connector, config=_config())
    monkeypatch.setattr(
        "nira.tools.weather.get_tool_credential",
...
def test_default_connector_path_honors_runtime_nira_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NIRA_HOME", str(root))
    monkeypatch.setattr(
        "nira.tools.weather.get_tool_credential",
        lambda *_args, **_kwargs: None,
    )
...
def test_missing_credentials_fails_without_calling_provider(monkeypatch):
    ...
    with patch("nira.tools.weather.weather_connector.fetch_weather") as fetch:
        result = tool.execute(location="Vienna")
...
def test_http_status_error_is_credential_safe():
    ...
    with patch("nira.connectors.weather.httpx.get", return_value=response):
...
def test_request_error_is_credential_safe():
    ...
    with patch(
        "nira.connectors.weather.httpx.get",
...
def test_weather_tool_handles_provider_error_after_connector_registry_repopulation(
    ...
    from nira.server.connectors_router import _ensure_connectors_registered
