...
``nira_rust`` extension is not required.  The mock simulates Rust
...
from nira.tools.git_tool import (
    GitCommitTool,
    GitDiffTool,
    GitLogTool,
    GitStatusTool,
...
def _make_mock_rust(tmp_path=None):
    """Return a mock module mimicking ``nira_rust`` git tools.
...
class TestGitStatusTool:
    ...
    def test_clean_repo(self, tmp_path):
        ...
        with patch("nira.tools.git_tool.get_rust_module", return_value=mock_mod):
...
class TestGitCommitTool:
    ...
    def test_git_not_found(self):
        tool = GitCommitTool()
        with patch("nira.tools.git_tool.shutil.which", return_value=None):
...
class TestCliFallbackWhenRustMissing:
    ...
    def _patch_no_rust(self):
        return patch(
            "nira.tools.git_tool.get_rust_module",
            side_effect=ImportError("No module named 'nira_rust'"),
        )
