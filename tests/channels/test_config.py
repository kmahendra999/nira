...
from nira.core.config import ChannelConfig, NiraConfig, load_config
...
class TestChannelConfigInNiraConfig:
    def test_channel_config_in_nira_config(self) -> None:
        cfg = NiraConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_nira_config_channel_defaults(self) -> None:
        cfg = NiraConfig()
