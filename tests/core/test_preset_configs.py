...
Presets are installed via `nira init --preset <name>`, which copies
`configs/nira/examples/<name>.toml` to `~/.nira/config.toml`.
...
PRESETS_DIR = Path(__file__).resolve().parents[2] / "configs" / "nira" / "examples"
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `nira init` expects to be populated for a working first run.
