"""Tests for intent-based agent routing in NiraSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal NiraSystem instance for testing _detect_agent_intent."""
        from nira.system import NiraSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = NiraSystem.__new__(NiraSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("nira.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_nira_triggers_digest(self, system):
        with patch("nira.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Nira")
