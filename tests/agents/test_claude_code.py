...
from nira.agents.claude_code import (
    _OUTPUT_END,
    _OUTPUT_START,
    _RUNNER_SRC,
    ClaudeCodeAgent,
...
class TestEnsureRunner:
    ...
    def test_creates_runner_dir(self, tmp_path, monkeypatch):
        ...
        # which checks $NIRA_HOME before ever falling back to
        # Path.home() -- so that's the env var to control here, not a
        # Path.home() patch (which get_config_dir() never even calls once
        # NIRA_HOME is set, e.g. by conftest.py's session-wide
        # isolation fixture).
        home_dir = tmp_path / "home"
        home_dir.mkdir()
        monkeypatch.setenv("NIRA_HOME", str(home_dir))
