...
from nira.agents.tool_resolver import ResolvedAgentTools
...
from nira.core.types import Role, ToolResult
...
def test_executor_closes_resolver_resources_when_pre_run_setup_fails(
    ...
    monkeypatch.setattr("nira.agents.executor.resolve_agent_tools", _resolve)
    system = SimpleNamespace(
        engine=FakeEngine([{"content": "unused"}]),
        model="test-model",
        memory_backend=None,
