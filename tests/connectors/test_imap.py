...
from nira.connectors.imap import (
    IMAPConnector,
    _create_pinned_socket,
    resolve_imap_host,
    validate_imap_endpoint,
)
from nira.connectors.oauth import load_tokens
...
def test_endpoint_policy_rejects_local_and_nonpublic_hosts(
    ...
    with (
        patch(
            "nira.connectors.imap.socket.getaddrinfo",
...
def test_endpoint_policy_rejects_mixed_public_private_dns() -> None:
    ...
    with (
        patch("nira.connectors.imap.socket.getaddrinfo", return_value=answers),
        pytest.raises(ValueError, match="non-public"),
    ):
...
def test_endpoint_policy_returns_only_checked_public_addresses() -> None:
    ...
    with patch("nira.connectors.imap.socket.getaddrinfo", return_value=answers):
...
def test_pinned_socket_connects_to_checked_ip_not_hostname() -> None:
    sentinel = object()
    with patch(
        "nira.connectors.imap.socket.create_connection",
...
def test_starttls_is_required_before_login() -> None:
    ...
    with (
        patch(
            "nira.connectors.imap.validate_imap_endpoint",
            return_value=("mail.example.com", ("93.184.216.34",)),
        ),
        patch("nira.connectors.imap._PinnedIMAP4", return_value=client),
    ):
...
def test_implicit_tls_uses_checked_ip_and_original_hostname() -> None:
    ...
    with (
        ...
        patch(
            "nira.connectors.imap.ssl.create_default_context",
            return_value=context,
        ),
        patch(
            "nira.connectors.imap._PinnedIMAP4SSL",
            return_value=client,
        ) as pinned_tls,
    ):
