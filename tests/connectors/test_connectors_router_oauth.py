...
``~/.nira/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    *source* (``nira.core.config``) — every connector re-derives
    ...
    import nira.connectors.oauth as oauth_mod
    import nira.core.config as config_mod
    import nira.server.connectors_router as router_mod
    ...
    google_mods = [
        "nira.connectors.gdrive",
        "nira.connectors.gcalendar",
        "nira.connectors.gcontacts",
        "nira.connectors.gmail",
        "nira.connectors.google_tasks",
        ...
        "nira.connectors.spotify",
        "nira.connectors.strava",
    ]
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("nira.core.open_browser") as mock_browser:
        resp = client.post(
...
def test_oauth_callback_exchanges_and_connects(
    ...
    from nira.connectors.gdrive import GDriveConnector
