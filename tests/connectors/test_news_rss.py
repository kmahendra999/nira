...
def test_sync_rss_feed(connector):
    ...
    with patch(
        "nira.connectors.news_rss._fetch_feed",
...
def test_fetch_feed_rechecks_redirect_target():
    from nira.connectors.news_rss import _fetch_feed
    ...
    with (
        patch(
            "nira.connectors.news_rss._validate_feed_url",
            side_effect=[MagicMock(), ValueError("RSS feed URL is not allowed")],
        ),
        patch(
            "nira.connectors.news_rss._request_feed",
...
def test_fetch_feed_pins_the_verified_address_against_dns_rebinding():
    ...
    with (
        patch("nira.security.ssrf.check_ssrf", return_value=None),
        patch(
            "nira.connectors.news_rss.socket.getaddrinfo",
            side_effect=[public_answer, rebound_private_answer],
        ) as resolve,
        patch(
            "nira.connectors.news_rss._PinnedHTTPConnection",
            _Connection,
        ),
    ):
...
def test_fetch_feed_rejects_private_answer_after_initial_ssrf_check():
    ...
    with (
        ...
        patch("nira.connectors.news_rss._PinnedHTTPConnection") as connection,
...
def test_https_connection_uses_url_hostname_for_tls_sni():
    from nira.connectors.news_rss import _PinnedHTTPSConnection
    ...
    with patch(
        "nira.connectors.news_rss.socket.create_connection",
...
def test_configure_writes_private_atomic_config(tmp_path):
    ...
    with (
        patch("nira.security.ssrf.check_ssrf", return_value=None),
        patch(
            "nira.connectors.news_rss._resolve_host_addresses",
