...
def connector(tmp_path: Path):
    ...
    from nira.connectors.gcalendar import GCalendarConnector  # noqa: PLC0415

    creds_path = str(tmp_path / "gcalendar.json")
    with patch(
        "nira.connectors.oauth._SHARED_GOOGLE_CREDENTIALS_PATH",
...
@patch("nira.connectors.gcalendar._gcal_api_calendars_list")
@patch("nira.connectors.gcalendar._gcal_api_events_list")
...
def test_parse_event_timestamp_handles_all_day_events() -> None:
    ...
    from nira.connectors.gcalendar import _parse_event_timestamp  # noqa: PLC0415
