...
A set of developer-focused scripts that use Nira tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Nira` instance with the `native_react` (ReAct) agent
...
1. **Install Nira** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/nira/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/nira/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run nira eval list` or
inspect `src/nira/tools/` for the full registry.
