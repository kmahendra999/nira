...
using Nira. The assistant uses an orchestrator agent loop with web
...
- Nira installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `nira eval`
runner.
...
User query
  |
  v
Nira SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `nira agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `nira.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/nira/recipes/) -- composable TOML configs that
