...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: nira channel list   to see available channels.\n",
    )
    ...
    click.echo("  from nira import Nira")
    click.echo(f'  j = Nira(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: nira channel status  (to verify "{channel}" is connected)')
