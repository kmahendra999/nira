...
Nira orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting Nira to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Nira installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Nira channel docs).
...
Nira supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/nira/config.toml` for the full list of channel and agent
options.
...
this via the Nira scheduler:

```bash
nira scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/nira/recipes/data/operators/` for examples.
