...
are local fakes, so you can ``python examples/nira/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/nira/examples/spec-search-quickstart.toml``
via the regular ``nira.core.config.load_config`` machinery — you can copy
that TOML to ``~/.nira/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    NIRA_HOME=/tmp/nira-spec-search-demo \\
...
from nira.core.config import SpecSearchLearningConfig
from nira.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from nira.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from nira.learning.spec_search.multi_session import SpecSearchLoop
from nira.learning.spec_search.orchestrator import SpecSearchOrchestrator
from nira.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from nira.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = nira.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.nira/config.toml``.
    ...
    #
    # (after copying ``configs/nira/examples/spec-search-quickstart.toml``
    # to ``~/.nira/config.toml``)
    ...
    home = Path(
        os.environ.get("NIRA_HOME")
        or tempfile.mkdtemp(prefix="nira-spec-search-")
    )
    print(f"NIRA_HOME = {home}")
    ...
    with patch(
        "nira.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
