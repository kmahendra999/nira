...
This tutorial demonstrates how to use Nira to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Nira` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `nira scheduler` CLI
...
Register each script as a recurring task with `nira scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | nira channel send slack
...
See `nira channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Nira auto-select from what is available.
