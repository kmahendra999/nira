...
TOPICS = [
    # Narrative — opinionated takes that happen to be about Nira
    ...
    "open source Apache local-first personal AI research",
...
FACTS = [
    ...
    "The Intelligence Per Watt study behind the upstream project found that local language models can accurately service 88.7 percent of single-turn chat and reasoning queries at interactive latencies, with intelligence efficiency improving 5.3 times from 2023 to 2025.",
    "In nearly all personal AI projects today, the local component is a thin orchestration layer, while the brain lives in someone else data center. Your most personal data routes through cloud APIs, with their latency, their cost, and their terms of service. Nira exists to fix this.",
    "Nira is structured around five composable primitives: Intelligence, Engine, Agents, Tools and Memory, and Learning. Each primitive can be benchmarked, substituted, and optimized independently.",
    "Nira includes hardware-agnostic telemetry that profiles energy consumption across NVIDIA GPUs, AMD GPUs, and Apple Silicon. Energy and dollar cost are first-class design constraints alongside accuracy.",
    ...
    "Nira is open source under Apache 2.0, and is a fork of OpenJarvis, which was built at Stanford at Hazy Research and the Scaling Intelligence Lab at SAIL. Because the tools for studying and building local-first AI should be available to everyone.",
...
def main() -> None:
    ...
    j = Nira(model="qwen3:32b", engine_key="ollama")
    j._config.agent.max_turns = 5

    # Clean and index
    db_path = os.path.expanduser("~/.nira/memory.db")
    ...
    parent_ts = slack_send(
        "*Nira Twitter Bot — Overnight Preview*\n"
    ...
    while True:
        ...
        tool_log.clear()
        r = j.ask_full(
            "You are @NiraAI — a researcher/dev who believes local-first "
            ...
            'server. we built nira to fix that"\n'
